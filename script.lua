...
-- v22.29.3 REAL DEEP FARM FIX
-- Root-cause fixes from Roblox-2026-08-20T21_08_37.014Z.mp4 + kaiv2_fixed (3):
-- 1) Exact active kaiv2 4-argument RegisterHit token shape, fresh RegisterAttack per victim.
-- 2) One persistent quest pile: ownership request + BodyVelocity/BodyPosition/BodyGyro + bounded CFrame snap.
-- 3) Reset stale HP/packet watch immediately when the sticky primary changes/dies.
-- 4) Player follows the persistent pile instead of chasing one mob away from the cluster.
-- 5) HP-delta fallback remains authoritative; pcall success is never treated as damage proof.
-- v22.29.4 REAL OLD BRING RESTORE
-- Bring restored from user's old source:
...
-- v22.29.7 REAL SINGLE FARM OWNER FIX
-- Deep audit result:
-- * v22.29.6 passed a new mob CFrame to TravelManager every tick -> AtomicRetarget spam.
-- * when no mob existed for one tick, Teddy returned false -> legacy ClusterFarm took control.
-- This build makes normal quest farm single-owner and uses live HRP Instance travel.
-- Net.RemoteEvent wrapper is preferred before raw RE/* children.
-- Secondary mobs enter attack only after their bring position persists for >=0.12s.
-- v22.29.8 REAL SINGLE COMBAT ENGINE FIX
-- Static root-cause audit of the user-supplied 19,441-line source:
-- * ReferenceVideoAttack and CombatController were both dispatching/owning combat state.
-- * A zero-HP direct-4 cycle could fall back into direct-4 again instead of rotating.
...
-- v22.31.0 REAL ROUTE + BRING HANDSHAKE FIX
-- Video Roblox-2026-08-21T00_52_31 confirms Lv300 Military Soldier quest can
-- remain 0/7 while Farm transitions away from Gladiator. QDB Magma coords are correct.
-- [R31-1] Quest-mob change hard-stops old Farm travel + pending combat + old bring state.
-- [R31-2] No-primary on a new/far field always routes directly to current q.MC; no old-death hold.
-- [R31-3] Once at q.MC, only a compact local field sweep is allowed; never previous-island patrol.
-- [B31-1] Every secondary gets request/set ownership BEFORE any movement probe.
-- [B31-2] owner=true pins normally; owner=nil gets one-shot persistence probe.
-- [B31-3] owner=false gets only one very-close post-handshake trial (REAL compatibility), never spam-pin.
-- [B31-4] unknown/false trial must persist twice and then produce real HP during probation or it is restored.
-- =================================================================
...
--  [V228-1] Removed per-victim RegisterAttack fanout. Roblox(29) shows one real kill while
--           later victims receive effects without equivalent kills; the older source itself
--           documents that one registered swing can be consumed by only the first victim
--           when several RegisterHit calls are fired from the same attack window.
--  [V228-2] Restored the reference combat shape: ONE RegisterAttack(0) + ONE RegisterHit
--           for the whole current AOE batch. Primary/base-part is selected once per swing.
--  [V228-3] Reference-exact payload (base part + remaining hit tuples) is tried first.
--           If two causally observed batches produce zero real Humanoid HP delta, the code
--           tries the other currently-public full-list batch shape; real HP delta selects
--           the working shape. pcall success never counts as damage proof.
--  [V228-4] A pulled NPC that loses ownership is immediately restored to its captured
--           pre-pull CFrame, quarantined briefly, and only then becomes attackable again.
--           This mirrors the reference source's restore/ignore behavior and removes statues.
--  [V228-5] Primary is sticky until death/despawn/name change. The farm no longer re-runs
--           nearest-target selection every tick and accidentally moves the cluster anchor.
-- =================================================================
-- =================================================================
...
--  [LR231-2] Long-range dispatch is still accepted only by real Humanoid.Health delta; visual/packet success never counts.
--  [LR231-3] Seed phase stops Farm travel as soon as it enters the long-range window, so the player can remain far above/away.
--  [LR231-4] If a long-range seed probe produces no HP delta, Farm temporarily falls back to the normal 100-stud window instead of stalling forever.
--  [LR231-5] Proven anchor/secondary proof attacks use the same bounded long-range window; bring/quest/Saber/melee logic is otherwise unchanged.
--
--  v22.23.0 REAL-HIT SEED ANCHOR BRING:
--  [A230-1] No mandatory park/centroid gate. Farm first attacks ONE untouched real quest mob at its live position.
--  [A230-2] Bring stays OFF until that seed mob shows a REAL Humanoid.Health decrease; that exact mob becomes the anchor.
--  [A230-3] Other same-name mobs are pulled to the proven anchor, never to an arbitrary field/player point.
--  [A230-4] A pulled secondary is not fan-out eligible from CFrame/ownership/persistence alone; it must itself lose real HP.
--  [A230-5] Secondary no-damage timeout restores its pre-pull position and retries later; no immortal visual ghost remains.
--  [A230-6] Proven secondaries keep an HP-liveness lease; if damage stops they are revoked/restored automatically.
--  [A230-7] When the seed anchor dies, visual pulls are released and a fresh real mob must earn HP proof before bring restarts.
--
--  v22.22.9 PARK-FIRST SERVER-SAFE TRIO:
--  [P229-1] Remote bring is DISARMED while the player is travelling/respawning. It scans first, parks once at a server-safe coverage point, then arms the pull.
--  [P229-2] Trio selection is pruned until every original spawn position is inside the real server-valid attack sphere from the parked hover. No wide 3-mob visual pile.
--  [P229-3] owner=false/nil may use a visual soft pin only while its original server-shadow position is inside that safe sphere; real Humanoid HP delta remains mandatory proof.
--  [P229-4] Removes the old fill-to-3 fallback that could append a far mob outside the local cluster and create no-damage ghosts.
--  [P229-5] Verified mobs are never towed while the stand-off point is not reached. Reset/quest travel cannot remote-magnet from the spawn NPC or mid-flight.
--
...
--           from this exact source, not the stale v22.8 copy that was accidentally exported.
--  [A224-2] Quest identity is strict again: a readable quest containing another known
--           QDB mob can no longer pass merely because ActiveQuestMob was cached.
--  [A224-3] Normal farm requires a verified quest match; only the existing short UI
--           rebuild/accept lease may bridge an unreadable wrapper.
--  [A224-4] Saber Jungle buttons never count a missing/unstreamed Button as a successful
--           touch. Each checkpoint streams/retries the exact PlateN.Button before advancing.
--  [A224-5] Saber room state checks use the authoritative Door.CanCollide path first,
--           matching current public Saber implementations and avoiding descendant false positives.
--  [A224-6] Moving Trio no longer freezes Humanoid WalkSpeed/AutoRotate during bring.
--           Only owned/proven roots get collision-softened, reducing statue/animation failures.
--  [A224-7] Unknown-owner remote proof uses a longer multi-sample settle before promotion;
--           failed proofs still fall back to close ownership acquire and never fake 3/3.
--
--  v22.22.3 VIDEO25 / REMOTE BRING FIXES:
--  [R25-1] Moving Trio now tries all 3 same-name quest mobs remotely before flying to any one mob.
--  [R25-2] Explicit owner=true roots are pulled immediately in one batch after SimulationRadius refresh.
--  [R25-3] owner=false roots are NEVER CFramed remotely; a short ownership-settle window runs first,
--           then Farm falls back to close acquire only if server ownership does not transfer.
--  [R25-4] owner=nil environments use one far snap + TWO-PHASE no-rewrite persistence proof.
--           Failed proof is not attacked/verified and falls back to physical acquire, preventing ghost/statue mobs.
--  [R25-5] Unverified remote trials do not freeze WalkSpeed/AutoRotate/CanCollide.
--  [R25-6] Ground-Y hysteresis prevents moving trio from jumping floors at prison tunnels/stairs.
--  [R25-7] Quest/checkpoints, dodge, melee and gacha logic are otherwise unchanged.
--
...
_G.Settings = {
    ...
    TeddyTrioUseClusterStandPoint = false,
    ...
    TeddyTrioRemoteFirst = true,
    TeddyTrioRemoteMaxDistance = 1800,
    TeddyTrioRemoteOwnerSettle = 0.18,
    TeddyTrioRemoteProofTime = 0.30,
    TeddyTrioRemoteProofChecks = 4,
    TeddyTrioRemoteSecondHold = 0.22,
    TeddyTrioRemoteProofRadius = 10,
    TeddyTrioRemoteRejectRadius = 34,
    TeddyTrioRemoteRetry = 0.55,
    TeddyTrioRemoteMaxFails = 1,
    TeddyTrioAllowFalseOwnerSnap = true, -- only an unproven visual trial
    TeddyTrioNoDamageTimeout = 1.60,
    TeddyTrioGhostRetry = 1.20,
    TeddyTrioDamageProofTTL = 4.00,
    TeddyTrioStandOffRemoteOnly = false,
    TeddyTrioStandOffHeight = 30, -- compatibility only
    TeddyTrioStandOffArrival = 12,
    TeddyTrioServerSafeRange = 100, -- compatibility only; HP proof is authoritative
    TeddyTrioPullArmRadius = 15,
    TeddyTrioStandOffOwnerRetry = 0.20,
    TeddyTrioSeedHover = 24,
    TeddyTrioSeedAttackRange = 100,
    -- v22.23.1: bounded remote-hit window. Server reality is still proven only by HP delta.
    TeddyLongRangeHitEnabled = true,
    TeddyLongRangeAttackRange = 300,
    TeddyLongRangeFallbackRange = 100,
    TeddyLongRangeProbeTimeout = 1.20,
    TeddyLongRangeFallbackHold = 3.00,
    TeddyTrioSeedProofTimeout = 2.20,
    TeddyTrioAnchorHover = 24,
    TeddyTrioSecondaryProofTimeout = 1.35,
    TeddyTrioMemberNoDamageTimeout = 2.60,
    TeddyTrioSecondaryRetry = 1.10,
...
local function QuestMatches(mobName)
    ...
    if text then
        ...
        -- If the readable wrapper names another canonical QDB enemy, this is
        -- definitely the wrong quest. Never let a stale ActiveQuestMob override it.
        local db = _G.BobonQuestDB
        if type(db) == "table" then
            local best = nil
            for _, entry in ipairs(db) do
                local name = entry and entry.M
                if type(name) == "string"
                    and string.find(lower, string.lower(name), 1, true) then
                    if not best or #name > #best then best = name end
                end
            end
            if best then return string.lower(best) == wanted end
        end

        -- A readable English objective that does not contain the requested mob
        -- is also a hard mismatch. Localized/structurally unusual text remains nil
        -- and is handled only by the short accepted-session fallback below.
        if lower:find("defeat", 1, true) or lower:find("kill", 1, true) then
            return false
        end
    end
...
function CombatController:ResolveRemotes()
    ...
    -- v22.29.7: use the game's Net wrapper first.
    -- kaiv2 active combat obtains these exact live RemoteEvent objects.
    if net and net:IsA("ModuleScript") then
        ...
        if type(api) == "table" and type(api.RemoteEvent) == "function" then
            ...
            if okA and a then attack = a end

            local okH, h = pcall(function()
                return api:RemoteEvent("RegisterHit", true)
            end)
            if okH and h then hit = h end
        end
    end

    -- Only fallback to exposed raw children when wrapper lookup failed.
    if not attack then
...
function CombatController:CollectTargets(preferred, mobName, maxRange)
    ...
    local sharedFarmActive = _G.Settings.SharedSourceFarmMode ~= false
        ...
        and (_G.State.FState == "SHARED_ATTACK"
            or _G.State.FState == "SHARED_BRING_FARM"
            or _G.State.FState == "SEED_REAL_ATTACK"
            or _G.State.FState == "SEED_CONFIRMED"
            or _G.State.FState == "ANCHOR_ATTACK"
            or _G.State.FState == "SECONDARY_HP_PROOF"
            or _G.State.FState == "SECONDARY_CONFIRMED")
...
function CombatController:Attack(tool, kind, preferredModel, preferredHum, preferredRoot, mobName)
    ...
    local longRangeFarm = _G.State and _G.Settings.TeddyLongRangeHitEnabled ~= false
        and (tonumber(_G.State.LongRangeFarmAttackUntil) or 0) >= now
        and not candidateInputBackend
    local candidateRange = candidateInputBackend
        ...
        or (longRangeFarm
            and math.max(_G.Settings.FastAttackRange or 100,
                tonumber(_G.State.LongRangeFarmAttackRange)
                    or tonumber(_G.Settings.TeddyLongRangeAttackRange) or 300)
    ...
    local longRangeDispatch = _G.State and _G.Settings.TeddyLongRangeHitEnabled ~= false
        and (tonumber(_G.State.LongRangeFarmAttackUntil) or 0) >= now
        and not inputBackend
    local range = inputBackend
        and (_G.Settings.ClientAttackRange or 8)
        or (longRangeDispatch
            and math.max(_G.Settings.FastAttackRange or 100,
                tonumber(_G.State.LongRangeFarmAttackRange)
                    or tonumber(_G.Settings.TeddyLongRangeAttackRange) or 300)
            or (_G.Settings.FastAttackRange or 100))
...
function ClusterFarmController:SharedRelease(reason)
    ...
    self.SharedTrioPendingPos = setmetatable({}, {__mode="k"})
    self.SharedTrioLastFollow = setmetatable({}, {__mode="k"})
    ...
    -- v22.23: restore any visual-only secondary before forgetting its origin.
    ...
    self.SharedTrioShadowSafe = setmetatable({}, {__mode="k"})
    ...
    if reason ~= "QuestRouteChanged" then
        self.ReferenceVideoRouteMobName = nil
        self.ReferenceVideoRouteFallback = nil
        self.ReferenceVideoRouteChangedAt = 0
        self.ReferenceVideoRouteSweepIndex = 0
        self.ReferenceVideoRouteLastSweep = 0
    end
...
function ClusterFarmController:SharedTeddyRestack(forceScan)
    ...
    if type(self.SharedMobName) ~= "string" or self.SharedMobName == "" then return 0, 0 end
    ...
    local memberTimeout = math.max(1.2,
        tonumber(_G.Settings.TeddyTrioMemberNoDamageTimeout) or 2.60)
    local retryDelay = math.max(0.25,
        tonumber(_G.Settings.TeddyTrioSecondaryRetry)
            or tonumber(_G.Settings.TeddyTrioRetryDelay) or 1.10)
    ...
    self.SharedTrioLastFollow = self.SharedTrioLastFollow or setmetatable({}, {__mode="k"})
...
function ClusterFarmController:ReferenceVideoPrimary(mobName)
    ...
    if oldPrimary then
        ...
        if oldRoot then self.ReferenceVideoLastPrimaryPos = oldRoot.Position end
    ...
    self.ReferenceVideoPrimaryModel = best
    self.ReferenceVideoPrimaryName = best and mobName or nil
    return best
end
...
function ClusterFarmController:ReferenceVideoBring(mobName, primary)
    ...
    local radius = math.max(260,
        tonumber(_G.Settings.ReferenceVideoBringRadius) or 420)
    local unknownTrialRange = math.max(120,
        math.min(radius, tonumber(_G.Settings.ReferenceVideoUnknownOwnerTrialRange) or 220))
    local falseTrialRange = math.min(90, unknownTrialRange)
    local maxMobs = math.max(2,
        math.floor(tonumber(_G.MobM)
            or tonumber(_G.Settings.ReferenceVideoMaxTargets)
            or 10))

    local ownerSettle = 0.30
    local probeCheck1 = 0.12
    local probeCheck2 = 0.26
    local probationWindow = 0.85
    local provenLease = 2.75
    local snapInterval = 0.08
    local verifyRadius = 14
        ...
        if IsEnemyNamed(mob, mobName) then
            ...
            if hum and hum.Health > 0 and root and root.Parent and not root.Anchored then
                ...
                if okPos and IsValidPos(pos) and IsAllowedWorldPosition(pos)
                    ...
                    rows[#rows + 1] = {
                        Model=mob, Humanoid=hum, Root=root,
                        Position=pos, Distance=(pos-center).Magnitude,
                    }
                end
            end
        end
    ...
    local moved, stable, blocked, probing = 0, 0, 0, 0

    local function restore(state, root)
        if state and state.OriginCF and root and root.Parent then
            pcall(function()
                root.CFrame = state.OriginCF
                root.AssemblyLinearVelocity = Vector3.zero
                root.AssemblyAngularVelocity = Vector3.zero
    ...
    local function oneShotProbe(state, root)
        state.OriginCF = state.OriginCF or root.CFrame
    ...
    for i=1,count do
        ...
        if mob == primary then
            ...
            if type(state)~="table" then
                state={
                    Phase="IDLE", NextRetry=0, LastSnap=0,
                    LastHealth=tonumber(hum.Health) or 0,
                    DamageAt=0, OwnerRequestedAt=0,
                }
            ...
            elseif oldHp and hp > oldHp + 0.01 and state.Phase~="OWNED" then
                state.Phase="IDLE"
            ...
            -- Request ownership FIRST, then let the executor/server settle.
            if now - (state.OwnerRequestedAt or 0) >= 0.22 then
                state.OwnerRequestedAt=now
                pcall(function()
            ...
            elseif phase=="PROVEN" then
                if now>(state.LeaseUntil or 0) then
                    state.Phase="WAIT_OWNER"
                    state.OwnerWaitUntil=now+ownerSettle
                else
                    if now-(state.LastSnap or 0)>=snapInterval then
                        state.LastSnap=now
                        pcall(function()
            ...
            elseif phase=="WAIT_OWNER" then
                probing=probing+1
                if owner==true then
                    state.Phase="OWNED"
                elseif now >= (state.OwnerWaitUntil or 0) then
                    local d=(root.Position-me.Position).Magnitude
                    if owner==nil and d<=unknownTrialRange then
                        oneShotProbe(state,root)
                    elseif owner==false and d<=falseTrialRange then
                        -- REAL can report false even after request/set. Allow exactly one
                        -- close physical trial, never continuous false-owner pinning.
                        oneShotProbe(state,root)
                    else
                        state.Phase="REJECT"
                        state.NextRetry=now+0.65
                        blocked=blocked+1
                    end
                end
            ...
            elseif phase=="PROBE1" then
                ...
                if now-(state.ProbeAt or now)>=probeCheck1 then
                    if state.ProbePos and (root.Position-state.ProbePos).Magnitude<=verifyRadius then
                        state.Phase="PROBE2"
                    else
                        restore(state,root)
                        state.Phase="REJECT"; state.NextRetry=now+0.70
                    end
                end
            ...
            elseif phase=="PROBATION" then
                probing=probing+1
                local near=state.ProbePos and (root.Position-state.ProbePos).Magnitude<=verifyRadius
                if not near then
                    restore(state,root)
                    state.Phase="REJECT"; state.NextRetry=now+0.75
                elseif (state.DamageAt or 0)>=(state.ProbationAt or math.huge) then
                    state.Phase="PROVEN"; state.LeaseUntil=now+provenLease
                    stable=stable+1
            ...
            elseif phase=="REJECT" then
                blocked=blocked+1
                if now>=(state.NextRetry or 0) then
                    state.Phase="WAIT_OWNER"
                    state.OwnerWaitUntil=now+ownerSettle
                    state.OriginCF=nil
                end
                ...
                -- IDLE: ask for authority and wait before moving anything.
                state.Phase="WAIT_OWNER"
                state.OwnerWaitUntil=now+ownerSettle
                probing=probing+1
            end
    ...
    if _G.BobonDiagnostics then
        _G.BobonDiagnostics.Bring="OWNER-HANDSHAKE-V31"
        ...
        _G.BobonDiagnostics.BringProbation=probing
    end
...
function ClusterFarmController:ReferenceVideoAttack(mobName, primary)
    ...
    local hum = primary and primary:FindFirstChildOfClass("Humanoid")
    if not me or not root or not root.Parent or not hum or hum.Health <= 0 then
        return false, 0
    end

    -- CRITICAL v22.29.8:
    -- There is now exactly ONE combat engine.  The previous ReferenceVideoAttack
    -- fired its own kaiv2 packet while CombatController maintained a separate
    -- PendingBackend/HP-proof machine.  After zero HP delta, the "fallback" could
    -- select SKID-DIRECT-4 again forever.  We now let CombatController own packet
    -- selection, HP proof, failure cooldown and backend rotation end-to-end.
    self.ReferenceVideoSingleOwnerActive = mobName
    _G.State.FState = "SHARED_ATTACK"

    local attempted = Attack(primary, mobName) == true
    ...
    if folder then
        ...
        for _, mob in ipairs(folder:GetChildren()) do
            if IsEnemyNamed(mob, mobName) then
                ...
                if mh and mh.Health > 0 and mr and mr.Parent then
                    if mob == primary
                        or self:IsSharedAttackEligible(mob, primary) then
                        if (mr.Position - me.Position).Magnitude
                            <= (_G.Settings.FastAttackRange or 100) then
                            count = count + 1
                        end
                    end
                end
    ...
    if _G.BobonDiagnostics then
        _G.BobonDiagnostics.Targets = count
        _G.BobonDiagnostics.Net = CombatController.PendingBackend
...
function ClusterFarmController:TeddySequenceFarmTick(mobName, fallbackCF, statusPrefix)
    ...
    if not _G.State or _G.State.Mode ~= "Farming"
        or _G.State.ActiveActionToken ~= 0 then
        return false
    end
    ...
    -- v22.31 ROUTE EPOCH:
    -- Level/quest transitions (e.g. Gladiator -> Military Soldier) must hard-reset
    -- every old farm/bring/combat position before the new q.MC is used.
    local routeChanged = self.ReferenceVideoRouteMobName ~= mobName
    if routeChanged then
        if _G.State.IsTraveling and _G.State.MovementOwner == "Farm" then
            pcall(function() TravelManager:Stop("QuestRouteChanged") end)
        end
        pcall(function() CombatController:AbortPending("QUEST-ROUTE-CHANGE") end)
        self:SharedRelease("QuestRouteChanged")
        self.ReferenceVideoRouteMobName = mobName
        self.ReferenceVideoRouteChangedAt = tick()
        self.ReferenceVideoRouteFallback = fallbackCF
        self.ReferenceVideoRouteSweepIndex = 0
        self.ReferenceVideoRouteLastSweep = 0
        self.ReferenceVideoPrimaryModel = nil
        self.ReferenceVideoPrimaryName = nil
    ...
    elseif self.SharedMobName
        and string.lower(tostring(self.SharedMobName)) ~= string.lower(mobName) then
    ...
    self.SharedMobName = mobName
    self.ReferenceVideoSingleOwnerActive = mobName
    self.ReferenceVideoRouteFallback = fallbackCF or self.ReferenceVideoRouteFallback

    -- Kill every historical normal-farm pile writer. Raid/item controllers remain separate.
    self.SharedTeddyActive = false
    self.SharedTrioPullArmed = false
    self.SharedPileCFrame = nil
    self.SharedClassicCurrentPile = nil
    ...
    if not primary then
        ...
        local targetCF = self.ReferenceVideoRouteFallback or fallbackCF
        local targetPos = targetCF and (
            typeof(targetCF) == "CFrame" and targetCF.Position
            or (typeof(targetCF) == "Vector3" and targetCF)
        ) or nil

        local distToField = targetPos and (me.Position - targetPos).Magnitude or math.huge
        local routeAge = tick() - (tonumber(self.ReferenceVideoRouteChangedAt) or 0)

        -- CRITICAL: when a quest just changed and the new island is far away, NEVER
        -- wait at the previous mob's death position. Go directly to the new q.MC.
        if targetPos and (routeAge < 12 or distToField > 260) then
            _G.State.FState = "ROUTE_TO_FIELD"
            _G.State.ActionText = "Travel Field • " .. mobName
            _G.BobonStatus = ("%s: ROUTE • %s • %.0f studs"):format(
                prefix, tostring(mobName), distToField)

            if _G.State:CanRequestTravel() then
                local routeCF = typeof(targetCF) == "CFrame" and targetCF or CFrame.new(targetPos)
                TravelManager:Request(routeCF, "Farm", {
                    arrivalThreshold = 45,
                    combatHover = false,
                    persistent = true,
                    speed = math.max(tonumber(_G.Settings.FlySpeed) or 430, 430),
                })
            end
            pcall(function() ExpandSimulationRadius() end)
            return true
        ...
        -- At the correct field but no live mob is streamed: remain inside a compact
        -- local sweep around q.MC. Never fall back to a previous island/primary.
        if targetPos then
            local offsets = {
                Vector3.new(0, 24, 0),
                Vector3.new(110, 24, 0),
                Vector3.new(-110, 24, 0),
                Vector3.new(0, 24, 110),
                Vector3.new(0, 24, -110),
                Vector3.new(80, 24, 80),
                Vector3.new(-80, 24, -80),
            }
            if tick() - (self.ReferenceVideoRouteLastSweep or 0) >= 0.85 then
                self.ReferenceVideoRouteLastSweep = tick()
                self.ReferenceVideoRouteSweepIndex =
                    ((tonumber(self.ReferenceVideoRouteSweepIndex) or 0) % #offsets) + 1
            end
            local sweepPos = targetPos + offsets[self.ReferenceVideoRouteSweepIndex or 1]
            local sweepCF = CFrame.new(sweepPos)
            _G.State.FState = "LOCAL_FIELD_SWEEP"
            _G.State.ActionText = "Scanning Field • " .. mobName
            _G.BobonStatus = prefix .. ": local field sweep • " .. mobName

            if _G.State:CanRequestTravel() then
                TravelManager:Request(sweepCF, "Farm", {
                    arrivalThreshold = 18,
                    combatHover = false,
                    persistent = false,
                    speed = _G.Settings.ClusterFieldPatrolSpeed
                        or _G.Settings.FlySpeed or 400,
                })
            end
        else
            _G.State.FState = "SINGLE_WAIT_MOB"
    ...
    _G.State.FState = "SHARED_ATTACK"
    _G.State.ActionText = "Live Primary • " .. mobName
    self.ReferenceVideoLastPrimaryPos = root.Position

    -- One movement owner, dynamic Instance target. This cannot inherit stale q.MC.
    if _G.State:CanRequestTravel() then
        TravelManager:Request(root, "Farm", {
            arrivalThreshold = math.max(5,
                tonumber(_G.Settings.FarmArrivalThreshold) or 8),
            combatHover = true,
            persistent = true,
            hoverHeight = tonumber(_G.Settings.ReferenceVideoHoverHeight) or 20,
    ...
    local brought, candidates, stable, blocked =
        self:ReferenceVideoBring(mobName, primary)
    local attacked, hitCount =
        self:ReferenceVideoAttack(mobName, primary)

    me = HRP()
    local dist = me and (me.Position - root.Position).Magnitude or -1
    _G.BobonStatus =
        ("%s: CORE31 • %s • d%.0f • bring %d/%d stable%d • hit%d %s")
        :format(prefix, tostring(mobName), dist,
            tonumber(brought) or 0,
            tonumber(candidates) or 0,
            tonumber(stable) or 0,
            tonumber(hitCount) or 0,
            attacked and "ACTIVE" or "FOLLOW")
...
function ClusterFarmController:IsSharedAttackEligible(model, primaryTarget)
    ...
    if _G.Settings.SharedTeddyMode ~= false then
        ...
        local damageAt = self.SharedTrioDamageProven and self.SharedTrioDamageProven[root]
        local lastDamage = self.SharedTrioLastDamageAt and self.SharedTrioLastDamageAt[root]
        local anchorCF = self.SharedPileCFrame
        -- v22.23: visual CFrame/ownership/persistence is never enough.
        if not at or not damageAt or not lastDamage or not anchorCF then return false end
        local ttl = math.max(0.10, tonumber(_G.Settings.SharedTeddyVerifyTTL) or 0.35)
        if tick() - at > ttl then return false end
        local liveTTL = math.max(1.2, tonumber(_G.Settings.TeddyTrioMemberNoDamageTimeout) or 2.60)
        if tick() - lastDamage > liveTTL then return false end
        local ok, pos = pcall(function() return root.Position end)
...
function ClusterFarmController:SharedFarmTick(mobName, fallbackCF)
    ...
    if _G.Settings.TeddySequenceMode ~= false then
        -- v22.29.7 normal quest farming has exactly ONE owner.
        -- TeddySequenceFarmTick returns true even while waiting for spawn.
        ...
        local ok, err = xpcall(function()
            ...
            local function DoorIsOpen()
                ...
                if door:IsA("BasePart") then
                    local okDoor, open = pcall(function()
                        return door.CanCollide == false
                    end)
                    return okDoor and open == true or nil
                end
                -- Compatibility fallback only if a future update wraps Door in a Model.
                local direct = door:FindFirstChild("Part")
                    or door:FindFirstChild("Part", true)
                return PartLooksOpen(direct or door)
            end
            ...
            local function RunPlates()
                ...
                for pass = 1, 8 do
                    ...
                    for i = 1, 5 do
                        ...
                        local touched = false
                        for attempt = 1, 3 do
                            _G.BobonStatus = ("Saber 1/8 • button %d/5 • pass %d/8 • try %d/3")
                                :format(i, pass, attempt)

                            local button = ResolvePlateButton(i)
                            if not button then
                                -- The old code treated a fallback CFrame write as a successful
                                -- button touch even when PlateN.Button was not streamed. Move to
                                -- its known area only to stream it, then require the real part.
                                StableTouch(plateFallback[i], nil, 0.18)
                                local streamUntil = tick() + 1.6
                                while _G.State:IsActionValid(myToken) and IsAlive()
                                    and tick() < streamUntil and not button do
                                    TouchAction()
                                    task.wait(0.10)
                            ...
                            if button and button.Parent then
                                touched = StableTouch(button.CFrame, button,
                                    attempt == 1 and 0.52 or 0.70)
                        ...
                        if not touched then
                            incompletePass = true
                            _G.BobonStatus = ("Saber 1/8 • button %d not streamed/touched • retrying")
                                :format(i)
                            break
                        end
                        task.wait(0.20)
                    ...
                    if incompletePass then
                        -- Keep the same Saber ActionToken and re-stream the Jungle rather
                        -- than releasing the quest after a partial plate sequence.
