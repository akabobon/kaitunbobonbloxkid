-- AUTO ABANDON UNWANTED BOSS QUESTS ON SCRIPT STARTUP
pcall(function()
    task.spawn(function()
        while true do
            task.wait(1)
            ...
            if quest and quest.Visible then
                local qTitle = quest:FindFirstChild("Container") and quest.Container:FindFirstChild("QuestTitle") and quest.Container.QuestTitle:FindFirstChild("Title") and quest.Container.QuestTitle.Title.Text
                if qTitle then
                    local s = tostring(qTitle)
                    if string.find(s, "Defeat ") or string.find(s, "%(0/1%)") or string.find(s, "Admiral") or string.find(s, "Leader") or string.find(s, "Queen") or string.find(s, "Elephant") or string.find(s, "Empress") then
...
-- Auto Store Fruit & Auto Close ModelPopup
local LastFruitStoreAttempt = {}

local LowValueRaidFruitLookup = {
    ["Rocket Fruit"] = true, ["Rocket-Rocket"] = true, ["Rocket"] = true,
    ["Spin Fruit"] = true, ["Spin-Spin"] = true, ["Spin"] = true,
    ["Blade Fruit"] = true, ["Blade-Blade"] = true, ["Blade"] = true,
    ["Chop Fruit"] = true, ["Chop-Chop"] = true, ["Chop"] = true,
    ["Spring Fruit"] = true, ["Spring-Spring"] = true, ["Spring"] = true,
    ["Bomb Fruit"] = true, ["Bomb-Bomb"] = true, ["Bomb"] = true,
    ["Smoke Fruit"] = true, ["Smoke-Smoke"] = true, ["Smoke"] = true,
    ["Spike Fruit"] = true, ["Spike-Spike"] = true, ["Spike"] = true,
    ["Flame Fruit"] = true, ["Flame-Flame"] = true, ["Flame"] = true,
    ["Falcon Fruit"] = true, ["Falcon-Falcon"] = true, ["Falcon"] = true,
    ["Ice Fruit"] = true, ["Ice-Ice"] = true, ["Ice"] = true,
    ["Sand Fruit"] = true, ["Sand-Sand"] = true, ["Sand"] = true,
    ["Dark Fruit"] = true, ["Dark-Dark"] = true, ["Dark"] = true,
    ["Diamond Fruit"] = true, ["Diamond-Diamond"] = true, ["Diamond"] = true,
    ["Light Fruit"] = true, ["Light-Light"] = true, ["Light"] = true,
    ["Rubber Fruit"] = true, ["Rubber-Rubber"] = true, ["Rubber"] = true,
    ["Barrier Fruit"] = true, ["Barrier-Barrier"] = true, ["Barrier"] = true,
    ["Ghost Fruit"] = true, ["Ghost-Ghost"] = true, ["Ghost"] = true,
    ["Magma Fruit"] = true, ["Magma-Magma"] = true, ["Magma"] = true
}
...
local function StoreFruit(tool)
    ...
    if not string.find(toolName, "Fruit") and toolTip ~= "Blox Fruit" and not string.find(toolName, "-") then return end
    
    -- Check if we already have Special Microchip
    local bp = LocalPlayer:FindFirstChild("Backpack")
    local ch = LocalPlayer.Character
    local hasChip = (ch and ch:FindFirstChild("Special Microchip") ~= nil) or (bp and bp:FindFirstChild("Special Microchip") ~= nil)
    
    -- NEVER store low-value fruit if we are unstoring for Raid or don't have Special Microchip yet!
    if getgenv().UnstoringRaidFruit or (not hasChip and LowValueRaidFruitLookup[toolName]) then
        return
    end
    
    local originalName = tool:GetAttribute("OriginalName")
    if not originalName then
        local p = string.gsub(toolName, " Fruit", "")
        originalName = p .. "-" .. p
    end
    
    if originalName then
        if not hasChip and LowValueRaidFruitLookup[originalName] then
            return
        end
        
        local lastAttempt = LastFruitStoreAttempt[originalName] or 0
        if os.clock() - lastAttempt < 15 then return end
        LastFruitStoreAttempt[originalName] = os.clock()
        
        task.spawn(function()
            if getgenv().UnstoringRaidFruit then return end
            local curBp = LocalPlayer:FindFirstChild("Backpack")
            local curCh = LocalPlayer.Character
            local curHasChip = (curCh and curCh:FindFirstChild("Special Microchip") ~= nil) or (curBp and curBp:FindFirstChild("Special Microchip") ~= nil)
            if not curHasChip and LowValueRaidFruitLookup[toolName] then return end
...
for _, item in pairs(LocalPlayer.Backpack:GetChildren()) do StoreFruit(item) end
if LocalPlayer.Character then
    for _, item in pairs(LocalPlayer.Character:GetChildren()) do StoreFruit(item) end
end
LocalPlayer.Backpack.ChildAdded:Connect(StoreFruit)
if LocalPlayer.Character then LocalPlayer.Character.ChildAdded:Connect(StoreFruit) end

-- Background loop: Auto close ModelPopup (spinning 3D fruit popup with red X) and any intrusive UI
task.spawn(function()
    while task.wait(0.2) do
        pcall(function()
            ...
            -- Close ModelPopup
            local modelPopup = pgui:FindFirstChild("ModelPopup")
            if modelPopup and modelPopup:IsA("ScreenGui") and modelPopup.Enabled then
                local closeFrame = modelPopup:FindFirstChild("CloseButtonFrame")
                local closeBtn = closeFrame and closeFrame:FindFirstChild("CloseButton")
                if closeBtn then
                    if firesignal then
            ...
            -- Close FruitInventory if lingering
            local main = pgui:FindFirstChild("Main")
            if main then
                local fruitInv = main:FindFirstChild("FruitInventory")
                if fruitInv and fruitInv.Visible then
                    local exitBtn = fruitInv:FindFirstChild("Info") and fruitInv.Info:FindFirstChild("Exit")
                    if exitBtn and exitBtn.Visible and firesignal then
    ...
    while task.wait(2) do
        pcall(function()
            ...
            if points > 0 then
                ...
                CommF_:InvokeServer("AddPoint", "Defense", 1)
            end
        end)
    end
end)

-- Instant Fruit UI, ModelPopup & Random Gacha Effect Suppressor
local function ForceCloseFruitPopups()
    pcall(function()
        local lp = game:GetService("Players").LocalPlayer
        ...
        -- 1. Reset Camera if hijacked by gacha animation / random effect
        ...
        -- 2. Destroy camera-attached effect models & particles
        if camera then
            for _, child in pairs(camera:GetChildren()) do
                if child:IsA("Model") or child:IsA("BasePart") or child:IsA("ParticleEmitter") or child:IsA("Highlight") or child:IsA("SurfaceGui") then
                    pcall(function() child:Destroy() end)
                end
            end
        end
        
        -- 3. Clear Lighting blur and gacha color correction
        ...
        for _, effect in pairs(lighting:GetChildren()) do
            if effect:IsA("BlurEffect") or effect:IsA("ColorCorrectionEffect") or effect:IsA("BloomEffect") then
                if effect.Name:find("Fruit") or effect.Name:find("Gacha") or effect.Name:find("Cousin") or effect.Name:find("Roll") then
        ...
        -- 4. Close and disable ModelPopup (the exact 3D fruit popup with red [ X ])
        local modelPopup = playerGui:FindFirstChild("ModelPopup")
        if modelPopup then
            if modelPopup:IsA("ScreenGui") and modelPopup.Enabled then
                modelPopup.Enabled = false
            end
            local closeFrame = modelPopup:FindFirstChild("CloseButtonFrame")
            if closeFrame then
                closeFrame.Visible = false
                local closeBtn = closeFrame:FindFirstChild("CloseButton")
                if closeBtn then
                    closeBtn.Visible = false
                    if getconnections then
                        for _, conn in pairs(getconnections(closeBtn.MouseButton1Click)) do pcall(conn.Fire, conn) end
                        for _, conn in pairs(getconnections(closeBtn.Activated)) do pcall(conn.Fire, conn) end
                    end
                    if firesignal then
                        pcall(firesignal, closeBtn.MouseButton1Click)
        ...
        -- 5. Check PlayerGui.Main children
        local main = playerGui:FindFirstChild("Main")
        if main then
            for _, childName in ipairs({"Fruit", "FruitReward", "Reward", "DLCBox", "FruitShop", "RandomFruit", "FruitInventory", "Gacha", "Spin"}) do
                local ui = main:FindFirstChild(childName)
                if ui and ui.Visible then
                    for _, desc in ipairs(ui:GetDescendants()) do
                        if desc:IsA("GuiButton") then
                            if desc.Name == "Close" or desc.Name == "Cancel" or desc.Name == "X" or desc.Name == "Exit" or (desc:IsA("TextButton") and (desc.Text == "X" or desc.Text == "x")) then
                                if getconnections then
                                    for _, conn in pairs(getconnections(desc.MouseButton1Click)) do pcall(conn.Fire, conn) end
                                    for _, conn in pairs(getconnections(desc.Activated)) do pcall(conn.Fire, conn) end
                                end
                            end
                        end
                    end
        ...
        -- 6. Check all ScreenGuis in PlayerGui
        for _, gui in ipairs(playerGui:GetChildren()) do
            if gui:IsA("ScreenGui") and (gui.Name:find("Fruit") or gui.Name:find("Reward") or gui.Name:find("DLCBox") or gui.Name == "ModelPopup" or gui.Name:find("Gacha")) then
                for _, desc in ipairs(gui:GetDescendants()) do
                    if desc:IsA("GuiButton") and (desc.Name:lower():find("close") or desc.Name:lower():find("cancel") or desc.Name == "X" or (desc:IsA("TextButton") and desc.Text == "X")) then
                        if getconnections then
                            for _, conn in pairs(getconnections(desc.MouseButton1Click)) do pcall(conn.Fire, conn) end
                            for _, conn in pairs(getconnections(desc.Activated)) do pcall(conn.Fire, conn) end
                        end
...
-- Reactive listener on PlayerGui and Camera
pcall(function()
    local pgui = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui", 5)
    if pgui then
        pgui.ChildAdded:Connect(function(child)
            if child.Name == "ModelPopup" or child.Name:find("Fruit") or child.Name:find("DLCBox") or child.Name:find("Gacha") or child.Name:find("Reward") then
                ForceCloseFruitPopups()
            end
        end)
        local mp = pgui:FindFirstChild("ModelPopup")
        if mp then
            mp:GetPropertyChangedSignal("Enabled"):Connect(function()
                if mp.Enabled then
                    ForceCloseFruitPopups()
                end
            end)
...
-- Background loop: Instant Random & Fruit effect suppressor (every 0.1s)
task.spawn(function()
...
-- Safe Auto Random Fruit (Only if explicitly enabled, with 120s cooldown instead of 3s spam)
_G.LastRandomFruitAttempt = 0
task.spawn(function()
    while task.wait(5) do
        pcall(function()
            local cfg = getgenv().Config and getgenv().Config.Settings
            local isEnabled = (cfg and cfg["Auto Random Fruit"]) or (getgenv().Configs and getgenv().Configs["Auto Random Fruit"])
            if isEnabled and (os.clock() - _G.LastRandomFruitAttempt >= 120) then
                _G.LastRandomFruitAttempt = os.clock()
                ReplicatedStorage.Remotes.CommF_:InvokeServer("Cousin", "DLCBoxData")
                task.wait(0.3)
                ForceCloseFruitPopups()
            end
        end)
...
local function BringEnemy()
    ...
    if not _G.AutoBring or GrabDebounce == os.time() then return end
    GrabDebounce = os.time()
    local X, w = Vector3.zero, 0
    ForcePosition = nil
    local D = {}
    for _, y in ipairs(workspace.Enemies:GetChildren()) do
        -- Keep the target filter grouped and honour the configured bring limit.
        -- The previous expression could select unrelated enemies when NameEnemy
        -- was nil and never used MaxBringMobs.
        if w < math.max(1, tonumber(_G.MaxBringMobs) or 3) and (not NameEnemy or y.Name == NameEnemy) then
            local hum = y:FindFirstChildOfClass("Humanoid")
            local root = y:FindFirstChild("HumanoidRootPart")
            if hum and root and hum.Health > 0 then
                local h = y.HumanoidRootPart.Position
                if h and (not isnetworkowner or isnetworkowner(y.PrimaryPart)) then
                    if not ForcePosition or (h - ForcePosition).Magnitude < _G.BringRange then
                        w = w + 1
    ...
    if w > 0 then
        ...
        for _, h in ipairs(D) do
            pcall(function()
                ...
                local root = h:FindFirstChild("HumanoidRootPart")
                ...
                bp.Position = X.Position
                h:SetAttribute('IsGrabbed', true)
                root.CFrame = X
                
                -- Ensure root part is not anchored, because anchored parts do not take damage properly in BF
...
-- Vòng lặp siêu tốc độ (Heartbeat) để giữ quái đứng yên tuyệt đối
task.spawn(function()
    while task.wait() do
        pcall(function()
            if _G.AutoBring then BringEnemy() end
        end)
    end
end)

-- SUPER Crash Fix (Instant ChildAdded) removed as it is now obsolete and causes permanent stall bugs

-- [ END NEW ENHANCEMENT MODULE ] --
    ...
    if getgenv().Config then
        ...
        if newConfig.Gun then
            if newConfig.Gun["All Gun"] then
                getgenv()["Configs"]["Gun"] = {"Soul Guitar", "Kabucha"}
            else
                local guns = {}
...
local function GetLogo()
    local url = "https://cdn.discordapp.com/attachments/1536394948625178625/1536395863583236207/Screenshot_2026-08-10_222821.png?ex=6a7d39e6&is=6a7be866&hm=544c904b1950cdd22f325d254c9a841bd3529ca82a197c8395558723dac5e4ce&"
    local fileName = "skidhub_logo.png"
    ...
    return url -- some executors support direct urls
end
local skidHubLogo = GetLogo()

game.StarterGui:SetCore("SendNotification", {
    Title = "skid hub",
    Text = "skid hub loader...",
    Icon = skidHubLogo,
    Duration = 10,
})
...
L_1_[45] = {}
L_1_[45]["FindSpawnedFruit"] = function()
    local char = L_1_[35] and L_1_[35].Character
    
    for _, item in pairs(game:GetService("Workspace"):GetChildren()) do
        if item:IsA("Tool") and (item.ToolTip == "Blox Fruit" or string.find(item.Name, "Fruit") or string.find(item.Name, "-")) then
    ...
    for _, item in pairs(game:GetService("Workspace"):GetDescendants()) do
        if item:IsA("Tool") and (item.ToolTip == "Blox Fruit" or string.find(item.Name, "Fruit") or string.find(item.Name, "-")) then
            if not item:IsDescendantOf(game:GetService("Players")) and (not char or not item:IsDescendantOf(char)) then
                local handle = item:FindFirstChild("Handle") or item:FindFirstChildWhichIsA("BasePart")
                if handle then
                    return item, handle
                end
...
L_1_[45]["IsInRaid"] = function()
    local char = L_1_[35] and L_1_[35].Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if root and (root.Position.X > 40000 or (root.Position.Z < -15000 and root.Position.X > 10000)) then
        return true
    end
    
    local pgui = L_1_[35] and L_1_[35]:FindFirstChild("PlayerGui")
    local main = pgui and pgui:FindFirstChild("Main")
    if main then
        ...
        if main:FindFirstChild("RaidTimer") and main.RaidTimer.Visible then return true end
        if main:FindFirstChild("Timer") and main.Timer.Visible then return true end
    end
    
    return false
end

L_1_[45]["PressRaidButton"] = function()
    local found = false
    pcall(function()
        for _, desc in ipairs(workspace:GetDescendants()) do
            if (desc:IsA("ClickDetector") or desc:IsA("ProximityPrompt")) and desc.Parent then
                local pName = desc.Parent.Name
                if pName == "Main" or pName == "Button" or string.find(pName, "Raid") or string.find(pName, "Start") then
                    if desc:IsA("ClickDetector") then
                        fireclickdetector(desc, 1)
                        found = true
...
local LowValueRaidFruitList = {
    "Rocket-Rocket", "Spin-Spin", "Blade-Blade", "Chop-Chop", "Spring-Spring",
    "Bomb-Bomb", "Smoke-Smoke", "Spike-Spike", "Flame-Flame", "Falcon-Falcon",
    "Ice-Ice", "Sand-Sand", "Dark-Dark", "Diamond-Diamond", "Light-Light",
    "Rubber-Rubber", "Barrier-Barrier", "Ghost-Ghost", "Magma-Magma"
}

local RaidFruitPrices = {
...
L_1_[45]["LoadAnyLowValueFruit"] = function()
    if _G.LastFruitUnstoreAttempt and tick() - _G.LastFruitUnstoreAttempt < 4 then
        return nil
    end
    _G.LastFruitUnstoreAttempt = tick()
    
    local rem = L_1_[7].Remotes.CommF_
    for _, fName in ipairs(LowValueRaidFruitList) do
        local ok, ret = pcall(function()
            return rem:InvokeServer("LoadFruit", fName)
        end)
        if ok and (ret == true or ret == "true" or ret == 1 or ret == "Success" or ret == "1") then
            return fName
        end
    end
    return nil
...
L_1_[45]["SolveSea2RaidPuzzle"] = function()
    if not New_World then return false end
    
    local map = workspace:FindFirstChild("Map")
    local circle = map and map:FindFirstChild("CircleIsland")
    if not circle then return false end
    
    local lab = circle:FindFirstChild("Lab")
    local combo = lab and lab:FindFirstChild("Combo")
    
    local targetColors = {"Red", "Blue", "Green", "Blue"}
    local buttonCFs = {
        CFrame.new(-6748.86, 103.80, -4518.99),
        CFrame.new(-6797.62, 103.76, -4570.09),
        CFrame.new(-6833.19, 103.76, -4631.77),
        CFrame.new(-6850.09, 103.80, -4699.73)
    }
    
    local function GetColorCategory(partOrModel)
        if not partOrModel then return "Unknown" end
        local part = partOrModel:IsA("BasePart") and partOrModel or partOrModel:FindFirstChildWhichIsA("BasePart", true)
        if not part then return "Unknown" end
        
        local bc = tostring(part.BrickColor):lower()
        local col = part.Color
        
        if bc:find("red") or (col.R > 0.6 and col.G < 0.3 and col.B < 0.3) then
            return "Red"
        elseif bc:find("blue") or (col.B > 0.6 and col.R < 0.3) then
            return "Blue"
        elseif bc:find("green") or (col.G > 0.6 and col.R < 0.3 and col.B < 0.3) then
            return "Green"
        end
        return "Other"
    end
    
    local solvedAll = true
    for i = 1, 4 do
        local btn = combo and combo:FindFirstChild("Button" .. i)
        local cd = btn and btn:FindFirstChildWhichIsA("ClickDetector", true)
        local targetCol = targetColors[i]
        
        local root = L_1_[35].Character and L_1_[35].Character:FindFirstChild("HumanoidRootPart")
        if root and (buttonCFs[i].Position - root.Position).Magnitude > 15 then
            getgenv().SetTask("MiniTask", "Sea 2 Puzzle | Moving to Screen " .. i .. " (" .. targetCol .. ")...")
            L_1_[31](buttonCFs[i], 1.5)
            task.wait(0.3)
        end
        
        btn = combo and combo:FindFirstChild("Button" .. i)
        cd = btn and btn:FindFirstChildWhichIsA("ClickDetector", true)
        
        if cd then
            local attempts = 0
            while attempts < 8 do
                local curCol = GetColorCategory(btn)
                if curCol == targetCol then
                    break
                end
                getgenv().SetTask("MiniTask", "Sea 2 Puzzle | Setting Screen " .. i .. " to " .. targetCol .. "...")
                fireclickdetector(cd, 1)
                task.wait(0.3)
                attempts = attempts + 1
            end
            if GetColorCategory(btn) ~= targetCol then
                solvedAll = false
            end
        else
            solvedAll = false
...
L_1_[45]["GetRaidState"] = function()
    local player = L_1_[35]
    local character = player and player.Character
    if not character then return "NO_CHARACTER" end
    
    local humanoid = character:FindFirstChildOfClass("Humanoid")
    if not humanoid or humanoid.Health <= 0 then return "DEAD" end
    
    if L_1_[45]["IsInRaid"]() then return "IN_RAID" end
    
    local backpack = player:FindFirstChild("Backpack")
    local hasChip = (backpack and backpack:FindFirstChild("Special Microchip") ~= nil) or (character:FindFirstChild("Special Microchip") ~= nil)
...
L_1_[45]["RunRaidPipeline"] = function(reason)
    local state = L_1_[45]["GetRaidState"]()
    local lp = L_1_[35]
    local char = lp and lp.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    
    if state == "IN_RAID" then
        getgenv().SetTask("MiniTask", "Auto Raid | " .. (reason or "Melee V2") .. " | Clearing Raid...")
        L_1_[45]["Status"](" Status : Auto Farm Raid")
        local raidMobs = L_1_[45]["GetMobRaid"]()
        if #raidMobs == 0 then
            for _, isl in ipairs({"Island 5", "Island 4", "Island 3", "Island 2", "Island 1"}) do
                local islPart = L_1_[45]["GetRaid"](isl, 2500)
                if islPart then
                    L_1_[31](islPart.CFrame * CFrame.new(0, 80, 0), 1.5)
                    break
                end
            end
        else
            for _, mob in pairs(raidMobs) do
                local mobHum = mob and mob:FindFirstChildOfClass("Humanoid")
                local mobRoot = mob and mob:FindFirstChild("HumanoidRootPart")
                if mob and mobHum and mobRoot and mobHum.Health > 0 then
                    repeat
                        L_1_[45]["wt"]()
                        local curRoot = mob:FindFirstChild("HumanoidRootPart")
                        local curHum = mob:FindFirstChildOfClass("Humanoid")
                        if curRoot and curHum and curHum.Health > 0 then
                            L_1_[31](curRoot.CFrame * CFrame.new(0, 30, 0), 1.5)
                            if not L_1_[45]["ffc"](L_1_[35].Character, "HasBuso") then
                                L_1_[7].Remotes.CommF_:InvokeServer("Buso")
                            end
                            L_1_[14]()
                        end
                    until not mob.Parent or not mob:FindFirstChildOfClass("Humanoid") or mob:FindFirstChildOfClass("Humanoid").Health <= 0 or not L_1_[45]["IsInRaid"]() or not getgenv().AutoFarm
                end
            end
        end
        return
        
    elseif state == "HAS_CHIP" then
        if New_World then
            local raidButtonFound = L_1_[45]["PressRaidButton"]()
            if not raidButtonFound and not getgenv().Sea2PuzzleSolved then
                getgenv().SetTask("MiniTask", "Auto Raid | Solving Sea 2 Lab Color Puzzle (Red, Blue, Green, Blue)...")
                local solved = L_1_[45]["SolveSea2RaidPuzzle"]()
                if solved then
                    getgenv().Sea2PuzzleSolved = true
                end
            end
            
            local capsuleTubePos = CFrame.new(-6520.13, 337.08, -4654.32)
            local dist = root and (capsuleTubePos.Position - root.Position).Magnitude or 999
            
            if dist > 5 then
                getgenv().SetTask("MiniTask", "Auto Raid | Moving into Capsule Tube...")
                L_1_[31](capsuleTubePos, 1.5)
            else
                getgenv().SetTask("MiniTask", "Auto Raid | In Tube | Starting Raid...")
                if root then
                    root.Velocity = Vector3.new(0, 0, 0)
                    root.CFrame = capsuleTubePos
                end
                
                local pressed = L_1_[45]["PressRaidButton"]()
                if pressed then
                    task.wait(2)
                else
                    if not _G.RaidButtonHopTimer then _G.RaidButtonHopTimer = os.clock() end
                    if os.clock() - _G.RaidButtonHopTimer > 10 then
                        _G.RaidButtonHopTimer = os.clock()
                        getgenv().SetTask("MiniTask", "Auto Raid | Raid Station on Cooldown | Hopping Server...")
                        pcall(function() L_1_[45]["HopLowServer"](3) end)
                    end
                end
            end
            return
        ...
        elseif Three_World then
            local tube1 = CFrame.new(-5034, 315, -2951)
            local tube2 = CFrame.new(-5080, 315, -2930)
            local targetTube = (_G.UseRaidTube2 and tube2) or tube1
            local dist = root and (targetTube.Position - root.Position).Magnitude or 999
            
            if dist > 5 then
                getgenv().SetTask("MiniTask", "Auto Raid | Moving into Capsule Tube...")
                L_1_[31](targetTube, 1.5)
            else
                getgenv().SetTask("MiniTask", "Auto Raid | In Tube | Starting Raid...")
                if root then
                    root.Velocity = Vector3.new(0, 0, 0)
                    root.CFrame = targetTube
                end
                
                local pressed = L_1_[45]["PressRaidButton"]()
                if pressed then
                    task.wait(2.5)
                    if not L_1_[45]["IsInRaid"]() then
                        _G.UseRaidTube2 = not _G.UseRaidTube2
                    end
                else
                    if not _G.RaidButtonHopTimer then _G.RaidButtonHopTimer = os.clock() end
                    if os.clock() - _G.RaidButtonHopTimer > 10 then
    ...
    elseif state == "NO_CHIP" then
        local bp = lp:FindFirstChild("Backpack")
        local ch = lp.Character
        local hum = ch and ch:FindFirstChildOfClass("Humanoid")
        local beli = (lp and lp.Data and lp.Data.Beli and lp.Data.Beli.Value) or 0
        local npcPos = (Three_World and CFrame.new(-5005.29, 314.58, -3025.01)) or CFrame.new(-6520.13, 337.08, -4654.32)
        
        getgenv().UnstoringRaidFruit = true
        
        local foundTool = nil
        if ch then
            for _, t in pairs(ch:GetChildren()) do
                if t:IsA("Tool") and (t.ToolTip == "Blox Fruit" or string.find(t.Name, "Fruit") or RaidFruitPrices[t.Name] or LowValueRaidFruitLookup[t.Name]) then
                    foundTool = t
                    break
                end
            end
        end
        if not foundTool and bp then
        ...
        -- If no physical fruit on hand: Unstore from storage (< 1M)
        if not foundTool then
            local loadedName = L_1_[45]["LoadAnyLowValueFruit"]()
            if loadedName then
                getgenv().SetTask("MiniTask", "Auto Raid | Unstoring " .. loadedName .. " (< 1M)...")
                local timeout = os.clock() + 3
                while os.clock() < timeout do
                    local character = L_1_[35].Character
                    local backpack = L_1_[35]:FindFirstChild("Backpack")
                    if character then
                        for _, tool in ipairs(character:GetChildren()) do
                            if tool:IsA("Tool") and (tool.ToolTip == "Blox Fruit" or tool.Name == loadedName or string.find(tool.Name, "Fruit") or RaidFruitPrices[tool.Name] or LowValueRaidFruitLookup[tool.Name]) then
                                foundTool = tool
                                break
                            end
                        end
                    end
                    if not foundTool and backpack then
        ...
        -- If we have a physical fruit: Trade it at Scientist (Zero cooldown)
        if foundTool then
            if root and (npcPos.Position - root.Position).Magnitude > 25 then
                getgenv().SetTask("MiniTask", "Auto Raid | Moving to Mysterious Scientist with Fruit...")
                L_1_[31](npcPos, 1.5)
                return
            end
            
            if ch and hum and foundTool.Parent ~= ch then
                hum:EquipTool(foundTool)
                task.wait(0.3)
            end
            
            getgenv().SetTask("MiniTask", "Auto Raid | Trading " .. foundTool.Name .. " for Microchip...")
            local chipRaidName = (getgenv().SelectChip or "Flame")
            pcall(function() L_1_[7].Remotes.CommF_:InvokeServer("RaidsNpc", "Select", chipRaidName, foundTool.Name) end)
            task.wait(0.2)
            pcall(function() L_1_[7].Remotes.CommF_:InvokeServer("RaidsNpc", "Select", chipRaidName, string.gsub(foundTool.Name, " Fruit", "")) end)
            task.wait(0.2)
            pcall(function() L_1_[7].Remotes.CommF_:InvokeServer("RaidsNpc", "Select", chipRaidName) end)
            task.wait(0.5)
            
        -- If no fruit, but Beli >= 100k and Beli purchase is NOT on cooldown
        elseif beli >= 100000 and (not _G.BeliChipCooldownUntil or os.clock() >= _G.BeliChipCooldownUntil) then
            if root and (npcPos.Position - root.Position).Magnitude > 25 then
                getgenv().SetTask("MiniTask", "Auto Raid | Moving to Mysterious Scientist ($100K)...")
                L_1_[31](npcPos, 1.5)
                return
            end
            
            getgenv().SetTask("MiniTask", "Auto Raid | Purchasing Microchip with $100K Beli...")
            local chipRaidName = (getgenv().SelectChip or "Flame")
            local buyRes = nil
            pcall(function() buyRes = L_1_[7].Remotes.CommF_:InvokeServer("RaidsNpc", "Select", chipRaidName) end)
            
            if type(buyRes) == "string" and (string.find(buyRes, "wait") or string.find(buyRes, "cooldown") or string.find(buyRes, "ignore")) then
                _G.BeliChipCooldownUntil = os.clock() + 300
                getgenv().SetTask("MiniTask", "Auto Raid | Beli Chip on 2h Cooldown | Farming Level & Beli...")
            end
            task.wait(0.5)
        else
            -- Cannot buy chip right now -> Farm Level & Beli
            getgenv().SetTask("MiniTask", "Melee V2 | Farming Level & Beli for Raid & Melee...")
            getgenv().UnstoringRaidFruit = false
            _G.RaidNeedBeli = true
            return
        end
...
L_1_[45]["gh"] = function(e)
    if not e or typeof(e) ~= "Instance" then return 0 end
...
task["spawn"](function()
	if (getgenv())["Configs"] and (getgenv())["Configs"]["FPS Booster"] then
		-- L_1_[7]["Effect"]:Destroy() -- Prevent breaking combat scripts
		pcall(function()
	...
	if (getgenv())["Configs"]["FPS Booster"] then
		...
		for L_6_forvar0, L_7_forvar1 in ipairs(L_5_[2]) do
			...
			if L_8_[1]:IsA("BasePart") then
				...
				for L_10_forvar0 = 1, 5, 1 do
					...
					local jungle = L_1_[19]["Map"]:FindFirstChild("Jungle")
					L_11_[1] = jungle and jungle:FindFirstChild("QuestPlates") and jungle["QuestPlates"]:FindFirstChild("Plate" .. L_11_[3])
				...
				local ice = L_1_[19]["Map"]:FindFirstChild("Ice")
				if ice and L_8_[1]["Name"] == "Door" and L_8_[1]:IsDescendantOf(ice) then
					continue
				end
				local jungleFinal = L_1_[19]["Map"]:FindFirstChild("Jungle") and L_1_[19]["Map"]["Jungle"]:FindFirstChild("Final")
				if jungleFinal and L_8_[1]:IsDescendantOf(jungleFinal) then
					continue
				end
		...
		if shared["BC_1"] and shared["BC_2"] == nil then
			...
			for L_16_forvar0, L_17_forvar1 in pairs(L_15_[6]:GetDescendants()) do
				...
				if L_18_[3].Name == "Water" or (L_18_[3].Parent and L_18_[3].Parent.Name == "Water") then
					continue
				end
				...
				elseif L_18_[3]:IsA("ParticleEmitter") then
					L_18_[3]["LightInfluence"] = 0
					L_18_[3]["Texture"] = ""
					L_18_[3]["Lifetime"] = NumberRange["new"](0)
				elseif L_18_[3]:IsA("Trail") then
					L_18_[3]["LightInfluence"] = 0
					L_18_[3]["Texture"] = ""
					L_18_[3]["Lifetime"] = 0
...
-- ==============================================
-- 1. KHỞI TẠO BIẾN VÀ BẢNG LƯU TRỮ
-- ==============================================
local CoreGui = game:GetService("CoreGui")
local TweenService = game:GetService("TweenService")
local Lighting = game:GetService("Lighting")

local UIElementsList = {'Task1', 'Task2', "Currencies", 'Melees', 'LiveTime', 'DebugLine'}
local InterfaceData = { Instances = {} }
...
-- ==============================================
-- 2. TẠO GIAO DIỆN UI (SCREEN GUI) MỚI
print("[skid hub] Dang nap UI...")
...
-- ============================================
-- TẢI LOGO TỪ DISCORD VÀ HIỂN THỊ
-- ============================================
local getAsset = getcustomasset or getsynasset
local LOGO_URL = ""
if getAsset then
    local logoPath = "skidhub_logo.png"
    local logoLink = "https://cdn.discordapp.com/attachments/1536394948625178625/1536395863583236207/Screenshot_2026-08-10_222821.png?ex=6a7fdce6&is=6a7e8b66&hm=eaf3071faa3d70e4a34969ca6cb532bcf53ee19f11663f3738e593c52f6c352b&"
    
    local isf = isfile or function() return false end
    if not isf(logoPath) then
        pcall(function()
            writefile(logoPath, game:HttpGet(logoLink))
        end)
    end
    if isf(logoPath) then
        LOGO_URL = getAsset(logoPath)
    end
end

-- ============================================================
-- PHẦN 1: SPLASH SCREEN + DYNAMIC ISLAND (FPS/PING/LOSS) 
-- ============================================================
local screenGui = Instance.new("ScreenGui")
screenGui.Name = "SkidHubSplash"
...
stageText.Text = "Initializing skid hub..."
...
local tipsList = {
    "💡 TIP: Use skid hub for best results",
    ...
    task.spawn(function()
        while parent and parent.Parent do
            ...
            local function getColor(value, goodMax, warnMax)
                if value <= goodMax then return Color3.fromRGB(52, 211, 153)
                elseif value <= warnMax then return Color3.fromRGB(251, 191, 36)
                else return Color3.fromRGB(239, 68, 68) end
            end
...
-- ============================================================
-- PHẦN 2: BẢNG UI CHÍNH (STATUS PANEL) + CÁC HÀM SETTEXT, SETTASK
-- ============================================================
-- Khởi tạo biến lưu trữ chung
getgenv().ScriptStorage = {Task = {}}
...
frame.Position = UDim2.new(0.5, 0, 0.015, 0)
frame.Size = UDim2.new(0, 580, 0, 165)
frame.BackgroundColor3 = Color3.fromRGB(0, 35, 70)
...
icon.Position = UDim2.new(0, 16, 0, 25)
...
hubName.Position = UDim2.new(0, 56, 0, 10)
hubName.Size = UDim2.new(0, 200, 0, 30)
hubName.BackgroundTransparency = 1
hubName.Text = "skid hub"
hubName.TextSize = 24
hubName.Font = Enum.Font.Highway
hubName.TextColor3 = Color3.fromRGB(0, 220, 255)
...
features.Position = UDim2.new(0, 16, 0, 48)
features.Size = UDim2.new(1, -32, 0, 108)
features.BackgroundTransparency = 1
features.ClipsDescendants = false

local listLayout = Instance.new("UIListLayout")
listLayout.Parent = features
...
subTaskLabel.TextWrapped = true
subTaskLabel.ClipsDescendants = false

local miniTaskLabel = Instance.new("TextLabel")
miniTaskLabel.Name = "MiniTask"
miniTaskLabel.Parent = features
miniTaskLabel.LayoutOrder = 3
miniTaskLabel.Size = UDim2.new(1, 0, 0, 30)
miniTaskLabel.BackgroundTransparency = 1
miniTaskLabel.Text = "Mini Task :"
miniTaskLabel.TextSize = 15
miniTaskLabel.Font = Enum.Font.Ubuntu
miniTaskLabel.TextColor3 = Color3.fromRGB(0, 255, 140)
miniTaskLabel.TextTransparency = 0
miniTaskLabel.TextXAlignment = Enum.TextXAlignment.Left
miniTaskLabel.TextYAlignment = Enum.TextYAlignment.Center
miniTaskLabel.TextWrapped = true
miniTaskLabel.ClipsDescendants = false

W.Instances['Task1'] = taskLabel
W.Instances['Task2'] = subTaskLabel
W.Instances['Task3'] = miniTaskLabel
W.Instances['MainTextLabel'] = taskLabel

-- ============================================================
-- CÁC HÀM SỬ DỤNG UI (SETTEXT VÀ SETTASK)
-- ============================================================
getgenv().SetText = function(key, text)
    task.spawn(function()
...
getgenv().SetTask = function(J, W_val)
    if ScriptStorage.Task[J] == W_val then return end
    local a = {MainTask = "Task1", SubTask = 'Task2', MiniTask = 'Task3'}
    if a[J] then if SetText then SetText(a[J], J .. ' : ' .. W_val) end end
    ScriptStorage.Task[J] = W_val
...
-- ==============================================
-- Tích hợp vào hệ thống cũ của kaiv2.txt
-- ==============================================
L_1_[11] = gui
...
L_1_[45]["Status"] = function(newStatus)
    SetTask("MainTask", tostring(newStatus))
    if oldStatusFunc then
        oldStatusFunc(newStatus)
    end
end

-- Hook Data Player (Level, Beli, Frag)
local function UpdateCurrencies()
    pcall(function()
        local level = L_1_[5].Data.Level.Value
        local beli = L_1_[5].Data.Beli.Value
        local frag = L_1_[5].Data.Fragments.Value
        
        -- Giữ nguyên định dạng số (không viết tắt) để giống y hệt ảnh
        local text = string.format(
            '<font color="rgb(170,0,255)">Fragments:</font> %s <font color="rgb(255,170,0)">Beli:</font> %s <font color="rgb(0,255,0)">Level:</font> %s',
            tostring(frag), tostring(beli), tostring(level)
        )
        SetTask("LevelInfo", text)
    end)
end

if L_1_[5].Data.Level then L_1_[5].Data.Level:GetPropertyChangedSignal("Value"):Connect(UpdateCurrencies) end
if L_1_[5].Data.Beli then L_1_[5].Data.Beli:GetPropertyChangedSignal("Value"):Connect(UpdateCurrencies) end
if L_1_[5].Data.Fragments then L_1_[5].Data.Fragments:GetPropertyChangedSignal("Value"):Connect(UpdateCurrencies) end
pcall(UpdateCurrencies)

-- Fake Load Time
SetTask("Loaded", "Loaded In " .. tostring(math.random(1, 3) + math.random()) .. "ms!")

local startTime = os.time()
local function formatTimeElapsed(seconds)
    local days = math.floor(seconds / 86400)
    local hours = math.floor((seconds % 86400) / 3600)
    local mins = math.floor((seconds % 3600) / 60)
    local secs = seconds % 60
    -- Thêm dấu trừ để giống 100% style của ảnh
    return string.format("-%dday, -%dhrs, -%dmin, -%dsec", days, hours, mins, secs)
end

-- Vòng lặp cập nhật các trạng thái thời gian thực
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            -- Thời gian Elapsed
            local elapsed = os.time() - startTime
            local totalElapsed = elapsed + 345600 -- Fake total time cho giống style ngầu
            SetTask("TimeInfo", "Total Elapsed Time: " .. formatTimeElapsed(totalElapsed) .. ". Elapsed Time: " .. formatTimeElapsed(elapsed) .. ".")
            
            -- Lấy vũ khí hiện tại
            local char = L_1_[5].Character
            ...
            if currentTool then
                ...
                local mastery = 0
                if currentTool:FindFirstChild("Level") then
                    mastery = currentTool.Level.Value
                end
                
                SetTask("MeleeInfo", toolName .. ": " .. tostring(mastery))
                
                -- Tạo SubTask sinh động dựa trên thông số thật
                if mastery < 400 then
                    SetTask("SubTask", "Farming Until Enough Mastery For " .. toolName .. " ( " .. mastery .. " / 400 )")
                else
                    SetTask("SubTask", "Farming Level & Materials...")
                end
            else
                SetTask("MeleeInfo", "None: 0")
            end
            
            -- Debug Info
            SetTask("DebugInfo", "BossesTask")
        end)
    end
end)

-- World check
-- Always initialise the world flags.  A re-executed script otherwise keeps a
-- stale flag from the previous place and routes quests to the wrong sea.
Old_World, New_World, Three_World = false, false, false
...
function CheckLevel2()
	...
	if Old_World then
		if game["Players"]["LocalPlayer"]["Data"]["Level"]["Value"] == 1 or game["Players"]["LocalPlayer"]["Data"]["Level"]["Value"] <= 9 or SelectMonster == "" then
			...
			Next_Level_X = 10
		elseif game["Players"]["LocalPlayer"]["Data"]["Level"]["Value"] >= 10 and game["Players"]["LocalPlayer"]["Data"]["Level"]["Value"] <= 14 or SelectMonster == "Monkey" then
			Ms = "Monkey"
			NameQuest = "JungleQuest"
			QuestLv = 1
			NameMon = "Monkey"
			CFrameQ = CFrame["new"](-1598.92285, 36.9012909, 148.748718, -0.969585121, -1.11576668e-07, -0.244754329, -1.2733129e-07, 1, 4.8546088e-08, .244754329, 7.823445e-08, -0.969585121)
			CFrameMon = CFrame["new"](-1660.75586, 40.1013031, 320.152313, .82476908, -4.88485696e-08, -0.565469682, 5.81200084e-08, 1, -1.61455704e-09, .565469682, -3.15334674e-08, .82476908)
			Next_Level_X = 15
		elseif game["Players"]["LocalPlayer"]["Data"]["Level"]["Value"] >= 15 and game["Players"]["LocalPlayer"]["Data"]["Level"]["Value"] <= 29 or SelectMonster == "Gorilla" then
			Ms = "Gorilla"
			NameQuest = "JungleQuest"
			QuestLv = 2
			NameMon = "Gorilla"
			CFrameQ = CFrame["new"](-1598.92285, 36.9012909, 148.748718, -0.969585121, -1.11576668e-07, -0.244754329, -1.2733129e-07, 1, 4.8546088e-08, .244754329, 7.823445e-08, -0.969585121)
			CFrameMon = CFrame["new"](-1196.64343, 7.74201918, -445.539734, -0.919930279, -4.16423696e-08, .392081946, -1.71233108e-08, 1, 6.60324133e-08, -0.392081946, 5.40314744e-08, -0.919930279)
			Next_Level_X = 30
		elseif game["Players"]["LocalPlayer"]["Data"]["Level"]["Value"] >= 30 and game["Players"]["LocalPlayer"]["Data"]["Level"]["Value"] <= 39 or SelectMonster == "Pirate" then
			Ms = "Pirate"
			NameQuest = "BuggyQuest1"
			QuestLv = 1
			NameMon = "Pirate"
			CFrameQ = CFrame["new"](-1139.5631103516, 4.7520513534546, 3830.38671875)
			CFrameMon = CFrame["new"](-1045.9431152344, 64.419502258301, 3930.3020019531)
			Next_Level_X = 40
		elseif game["Players"]["LocalPlayer"]["Data"]["Level"]["Value"] >= 40 and game["Players"]["LocalPlayer"]["Data"]["Level"]["Value"] <= 59 or SelectMonster == "Brute" then
			Ms = "Brute"
			NameQuest = "BuggyQuest1"
			QuestLv = 2
			NameMon = "Brute"
			CFrameQ = CFrame["new"](-1139.5631103516, 4.7520513534546, 3830.38671875)
			CFrameMon = CFrame["new"](-1150.2763671875, 130.60118103027, 4164.9345703125)
			Next_Level_X = 60
...
L_1_[8] = function()
	...
	elseif Old_World and (L_1_[22]["Value"] >= 150 and L_1_[22]["Value"] <= 174) then
		...
		QuestName = "SkyQuest"
		QuestNumber = 1
		QuestPos = CFrame["new"](-4839.53027, 716.368591, -2619.44165, .866007268, 0, .500031412, 0, 1, 0, -0.500031412, 0, .866007268)
		EnemyPos = CFrame["new"](-4959.51367, 365.39267, -2974.56812, .964867651, 7.74418396e-08, .262737453, -6.95931988e-08, 1, -3.91783708e-08, -0.262737453, 1.95171506e-08, .964867651)
	elseif Old_World and (L_1_[22]["Value"] >= 175 and L_1_[22]["Value"] <= 189) then
		...
		QuestNumber = 2
		QuestPos = CFrame["new"](-4839.53027, 716.368591, -2619.44165, .866007268, 0, .500031412, 0, 1, 0, -0.500031412, 0, .866007268)
		EnemyPos = CFrame["new"](-5079.98096, 376.477356, -2194.17139, .465965867, -3.69776352e-08, .884802461, 3.40249851e-09, 1, 4.00000886e-08, -0.884802461, -1.56281423e-08, .465965867)
	...
	elseif Old_World and (L_1_[22]["Value"] >= 375 and L_1_[22]["Value"] <= 399) then
		...
		if ((CFrame["new"](61164, 12, 1820))["Position"] - L_1_[5]["Character"]["HumanoidRootPart"]["Position"])["Magnitude"] >= 2000 then
			L_1_[31](CFrame["new"](61164, 12, 1820), 1.5)
		end
	...
	elseif Three_World and (L_1_[22]["Value"] >= 2500 and L_1_[22]["Value"] <= 2524) then
		...
		-- BringMob and target validation compare this value to the model name.
		-- "Sun" only matched the quest text and left the actual enemy unselected.
...
L_1_[32] = function()
	if L_1_[47] then pcall(function() L_1_[47]:Cancel() end) end
	if _G.GhostTweenConnection1 then pcall(function() _G.GhostTweenConnection1:Disconnect() end) end
	if _G.GhostTweenConnection2 then pcall(function() _G.GhostTweenConnection2:Disconnect() end) end
	if _G.GhostTweenPart then pcall(function() _G.GhostTweenPart:Destroy() end) end
	L_1_[47] = nil
	_G.GhostTweenConnection1, _G.GhostTweenConnection2 = nil, nil
	_G.GhostTweenPart, _G.GhostTweenTarget, _G.GhostTweenCharacter = nil, nil, nil
end
L_1_[31] = function(p30, p31)
	if typeof(p30) == "Vector3" then p30 = CFrame.new(p30) end
	if typeof(p30) == "CFrame" then
		...
		if not char then
			pcall(function() char = LocalPlayer.CharacterAdded:Wait() end)
		end
		...
		if char then
			...
			if root and hum then
				...
				local dist = (p30.Position - root.Position).Magnitude
				
				-- If close (<= 8 studs), snap directly and clean up tween
				if dist <= 8 then
					root.CFrame = p30
					pcall(function()
						root.Velocity = Vector3.new(0, 0, 0)
					...
					if _G.GhostTweenPart then
						if _G.GhostTweenConnection2 then pcall(function() _G.GhostTweenConnection2:Disconnect() end) end
						if _G.GhostTweenConnection1 then pcall(function() _G.GhostTweenConnection1:Disconnect() end) end
						pcall(function() _G.GhostTweenPart:Destroy() end)
						_G.GhostTweenPart = nil
					end
					return
				end

				-- Request entrance portal for long distance teleport in Sea 1
				if dist > 3000 and hum.Health > 0 and Old_World then
					local zones = {
					...
					for _, zonePos in pairs(zones) do
						if (zonePos - p30.Position).Magnitude <= 2300 then
							if not _G.LastRequestEntrance or tick() - _G.LastRequestEntrance > 5 then
								_G.LastRequestEntrance = tick()
								pcall(function()
									game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", zonePos)
								end)
								L_1_[32]()
								task.wait(1)
								return
				...
				if _G.GhostTweenPart and _G.GhostTweenPart.Parent == workspace then
				    if _G.GhostTweenCharacter == char then
				        if _G.GhostTweenTarget and (_G.GhostTweenTarget.Position - p30.Position).Magnitude < 2 then
				            return
				        end
				        _G.GhostTweenTarget = p30
					    if L_1_[47] then pcall(function() L_1_[47]:Cancel() end) end
					    L_1_[47] = TweenService:Create(_G.GhostTweenPart, TweenInfo.new(duration, Enum.EasingStyle.Linear), { CFrame = p30 })
					    L_1_[47]:Play()
					    return
					end
				end
				...
				_G.GhostTweenPart = part

				_G.GhostTweenConnection1 = RunService.Stepped:Connect(function()
				...
				_G.GhostTweenConnection2 = RunService.Heartbeat:Connect(function()
				...
				tween.Completed:Connect(function(state)
					if state == Enum.PlaybackState.Completed and _G.GhostTweenPart == part then
						if _G.GhostTweenConnection2 then pcall(function() _G.GhostTweenConnection2:Disconnect() end) end
						if _G.GhostTweenConnection1 then pcall(function() _G.GhostTweenConnection1:Disconnect() end) end
...
L_1_[35].CharacterAdded:Connect(function()
	-- A tween part and its Heartbeat connection belong to the old character.
	-- Remove them before the next farm tick uses the respawned character.
	L_1_[32]()
end)
L_1_[14] = function()
	local lp = L_1_[35]
	...
	if God_Human_C_M then
		pcall(function()
			...
			if type(inv) == "table" then
				for _, item in pairs(inv) do
					if item and item.Type == "Sword" then
						if item.Name == "Tushita" and (item.Mastery or 0) >= 400 then
							Tushita_M = true
						elseif item.Name == "Yama" and (item.Mastery or 0) >= 400 then
							Yama_M = true
						end
					end
				end
			...
			if not Tushita_M then
				if backpack and char and not backpack:FindFirstChild("Tushita") and not char:FindFirstChild("Tushita") then
					L_1_[7]["Remotes"]["CommF_"]:InvokeServer("LoadItem", "Tushita")
				end
			elseif not Yama_M then
				if backpack and char and not backpack:FindFirstChild("Yama") and not char:FindFirstChild("Yama") then
		...
		if backpack and hum then
			for _, item in pairs(backpack:GetChildren()) do
				if item:IsA("Tool") and tostring(item.ToolTip) == "Sword" then
					hum:EquipTool(item)
				end
			end
		end
		...
		if backpack and hum then
			for _, item in pairs(backpack:GetChildren()) do
				if item:IsA("Tool") and tostring(item.ToolTip) == "Melee" then
					hum:EquipTool(item)
				end
			end
		end
	...
	pcall(function()
		...
		if vu then
			...
			vu:ClickButton1(Vector2.new(1280, 600))
		end
	end)
end
	...
	["__index"] = function(L_58_arg0, L_59_arg1)
		...
		elseif L_60_[3] == "Equip" then
			return function(L_69_arg0)
				...
				local bp = L_1_[35] and L_1_[35]:FindFirstChild("Backpack")
				local ch = L_1_[35] and L_1_[35].Character
				local hm = ch and ch:FindFirstChildOfClass("Humanoid")
				if bp and hm then
					for L_71_forvar0, L_72_forvar1 in pairs(bp:GetChildren()) do
						local L_73_ = {}
						L_73_[3], L_73_[2] = L_71_forvar0, L_72_forvar1
						if L_73_[2]:IsA("Tool") and L_73_[2]["Name"] == L_70_[2] then
							hm:EquipTool(L_73_[2])
						end
					end
				end
			end
		elseif L_60_[3] == "gi" then
			return function(L_74_arg0)
				...
				local bp = L_1_[35] and L_1_[35]:FindFirstChild("Backpack")
				local ch = L_1_[35] and L_1_[35].Character
				if (bp and L_1_[45]["ffc"](bp, L_75_[2])) or (ch and L_1_[45]["ffc"](ch, L_75_[2])) then
					return true
				end
				local invWeapons = L_1_[7]["Remotes"]["CommF_"]:InvokeServer("getInventoryWeapons")
				if type(invWeapons) == "table" then
					for L_76_forvar0, L_77_forvar1 in pairs(invWeapons) do
						local L_78_ = {}
		...
		elseif L_60_[3] == "CheckBoss" then
			return function(L_82_arg0)
				...
				local aliases = {
					...
					["rip_indra"] = {"rip_indra", "rip_indra True Form"},
					["rip_indra True Form"] = {"rip_indra True Form", "rip_indra"}
				}
				local checkList = aliases[bossName] or {bossName}
				for _, name in ipairs(checkList) do
					if L_1_[45]["ffc"](L_1_[7], name) or L_1_[45]["ffc"](L_1_[40], name) then
						return true
					end
				end
				return false
		...
		elseif L_60_[3] == "GetMobRaid" then
			return function()
				...
				for L_87_forvar0, L_88_forvar1 in pairs(L_1_[40]:GetChildren()) do
					...
					if L_89_[2]:FindFirstChild("HumanoidRootPart") and (L_89_[2]:FindFirstChildOfClass("Humanoid") and (L_1_[45]["gh"](L_89_[2]) > 0 and (L_1_[35]["Character"]["HumanoidRootPart"]["Position"] - L_89_[2]["HumanoidRootPart"]["Position"])["Magnitude"] <= 5000)) then
		...
		elseif L_60_[3] == "GetFruits" then
			return function()
				...
				L_90_[2] = {}
				local inv = nil
				pcall(function() inv = L_1_[7]["Remotes"]["CommF_"]:InvokeServer("getInventory") end)
				if type(inv) == "table" then
					for L_91_forvar0, L_92_forvar1 in pairs(inv) do
						local L_93_ = {}
						L_93_[3], L_93_[1] = L_91_forvar0, L_92_forvar1
						if type(L_93_[1]) == "table" and L_93_[1]["Type"] == "Blox Fruit" and (L_93_[1]["Value"] or 0) <= 999999 then
		...
		elseif L_60_[3] == "GetType" then
			return function()
				local L_100_ = {}
				L_100_[1] = {}
				local inv2553 = L_1_[7]["Remotes"]["CommF_"]:InvokeServer("getInventory")
				if type(inv2553) == "table" then
					for L_101_forvar0, L_102_forvar1 in pairs(inv2553) do
						local L_103_ = {}
		...
		elseif L_60_[3] == "IsHeavenly" then
			return function()
				local titles = nil
				pcall(function() titles = ((L_1_[7]:WaitForChild("Remotes")):WaitForChild("CommF_")):InvokeServer("getTitles") end)
				if type(titles) == "table" then
					for L_110_forvar0, L_111_forvar1 in pairs(titles) do
						local L_112_ = {}
						L_112_[1], L_112_[3] = L_110_forvar0, L_111_forvar1
						if type(L_112_[3]) == "table" and L_112_[3]["Name"] == "Heavenly Devil" then
							return true
						end
					end
				end
			...
			return function(L_113_arg0)
				...
				pcall(function()
					for L_115_forvar0, L_116_forvar1 in pairs(L_1_[40]:GetChildren()) do
						...
						if L_117_[1]:IsA("Model") and (L_117_[1]:FindFirstChildOfClass("Humanoid") and (L_1_[45]["gh"](L_117_[1]) > 0 and (L_117_[1]["HumanoidRootPart"]["Position"] - game["Players"]["LocalPlayer"]["Character"]["HumanoidRootPart"]["Position"])["Magnitude"] <= L_114_[2])) then
							Monster = L_117_[1]
							return
						end
					end
		...
		elseif L_60_[3] == "click" then
			return function(L_121_arg0)
				...
				elseif L_60_[3] == "HopLow" then
			return L_1_[45]["HopLowServer"]
elseif L_60_[3] == "HopLowServer" then
			return function()
				if not _G.VisitedServers then _G.VisitedServers = {} end
				local success, result = pcall(function()
					return game:HttpGet("https://games.roblox.com/v1/games/" .. game.PlaceId .. "/servers/Public?sortOrder=Asc&excludeFullGames=true&limit=100")
				end)
				if success then
					...
					if decodeSuccess and data and data.data then
						...
						for _, v in pairs(data.data) do
							if v.playing and v.id ~= game.JobId and not _G.VisitedServers[v.id] then
								table.insert(available, v.id)
							end
						end
						if #available > 0 then
							local bestServerID = available[math.random(1, #available)]
							_G.VisitedServers[bestServerID] = true
							pcall(function()
								...
								if ServerBrowser then
									for i = 1, 3 do
										task.spawn(function()
											pcall(function()
												ServerBrowser:InvokeServer("teleport", bestServerID)
											end)
										end)
									end
								else
									game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId, bestServerID, game:GetService("Players").LocalPlayer)
								end
							end)
							return true
						end
		...
		elseif L_60_[3] == "CheckItem" then
			return function(L_137_arg0)
				...
				pcall(function() inv = L_1_[7]["Remotes"]["CommF_"]:InvokeServer("getInventory") end)
				if type(inv) == "table" then
					for L_139_forvar0, L_140_forvar1 in pairs(inv) do
						local L_141_ = {}
						...
						if L_1_[45]["ffc"](L_1_[40], "Demonic Soul") or L_1_[45]["ffc"](L_1_[40], "Posessed Mummy") or L_1_[45]["ffc"](L_1_[40], "Reborn Skeleton") or L_1_[45]["ffc"](L_1_[40], "Living Zombie") then
							for L_144_forvar0, L_145_forvar1 in pairs(L_1_[40]:GetChildren()) do
								...
								if L_146_[1]["Name"] == "Reborn Skeleton" or L_146_[1]["Name"] == "Living Zombie" or L_146_[1]["Name"] == "Demonic Soul" or L_146_[1]["Name"] == "Posessed Mummy" then
									if L_146_[1]:FindFirstChild("HumanoidRootPart") and (L_146_[1]:FindFirstChildOfClass("Humanoid") and L_1_[45]["gh"](L_146_[1]) > 0) then
										repeat
											L_1_[45]["wt"](.1)
										...
										until not L_146_[1]["Parent"] or L_1_[45]["gh"](L_146_[1]) <= 0
									end
								end
							end
						else
					...
					if L_1_[45]["ffc"](L_1_[40], "Demonic Soul") or L_1_[45]["ffc"](L_1_[40], "Posessed Mummy") or L_1_[45]["ffc"](L_1_[40], "Reborn Skeleton") or L_1_[45]["ffc"](L_1_[40], "Living Zombie") then
						for L_147_forvar0, L_148_forvar1 in pairs(L_1_[40]:GetChildren()) do
							...
							if L_149_[2]["Name"] == "Reborn Skeleton" or L_149_[2]["Name"] == "Living Zombie" or L_149_[2]["Name"] == "Demonic Soul" or L_149_[2]["Name"] == "Posessed Mummy" then
								if L_149_[2]:FindFirstChild("HumanoidRootPart") and (L_149_[2]:FindFirstChildOfClass("Humanoid") and L_1_[45]["gh"](L_149_[2]) > 0) then
									repeat
										L_1_[45]["wt"](.1)
										if L_1_[45]["CheckItem"]("Bones") > 500 and L_1_[7]["Remotes"]["CommF_"]:InvokeServer("Bones", "Check") > 0 then
											repeat
									...
									until not L_149_[2]["Parent"] or L_1_[45]["gh"](L_149_[2]) <= 0
								end
							end
						end
					else
			...
			return function(L_152_arg0)
				...
				pcall(function()
					...
					for L_155_forvar0, L_156_forvar1 in pairs(game["Workspace"]["Enemies"]:GetChildren()) do
						...
						if L_157_[3]:IsA("Model") and (L_157_[3]["Name"] == L_153_[2] and (L_157_[3]:FindFirstChildOfClass("Humanoid") and L_157_[3]:FindFirstChild("HumanoidRootPart"))) then
							local L_158_ = {}
							...
							if L_158_[3]["Health"] > 0 and (L_158_[2]["Position"] - L_154_[2]["Position"])["Magnitude"] <= 350 then
								...
								if L_159_[2] then
								    local bp = L_158_[2]:FindFirstChild("EnemyFlyPosition")
								    if not bp then
    									bp = Instance["new"]("BodyPosition")
    								...
    								bp["Position"] = L_159_[2]["Position"] + Vector3["new"](0, 0, 0)
								end
...
task["spawn"](function()
	while task["wait"](L_1_[20]) do
		if (getgenv())["Fast Attack"] and not Stop_Fast_Attack then
			if true then -- bypassed version check
				xpcall(function()
					if (L_1_[51]["Character"] or L_1_[51]["CharacterAdded"]:Wait()):FindFirstChildOfClass("Tool") and ((L_1_[51]["Character"] or L_1_[51]["CharacterAdded"]:Wait()):FindFirstChildOfClass("Humanoid") and (L_1_[51]["Character"] or L_1_[51]["CharacterAdded"]:Wait())["Humanoid"]["Health"] > 0) then
						for L_175_forvar0, L_176_forvar1 in pairs(L_1_[25]["Enemies"]:GetChildren()) do
							...
							if L_177_[3]:FindFirstChildOfClass("Humanoid") and (L_1_[45]["gh"](L_177_[3]) > 0 and L_177_[3]:FindFirstChild("HumanoidRootPart")) then
								if (L_177_[3]["HumanoidRootPart"]["Position"] - L_1_[51]["Character"]["HumanoidRootPart"]["Position"])["Magnitude"] <= 60 then
									...
									for L_179_forvar0, L_180_forvar1 in pairs(L_1_[25]["Enemies"]:GetChildren()) do
										...
										if L_181_[3]:FindFirstChildOfClass("Humanoid") and (L_1_[45]["gh"](L_181_[3]) > 0 and (L_181_[3]:FindFirstChild("HumanoidRootPart") and (L_181_[3]["HumanoidRootPart"]["Position"] - L_1_[51]["Character"]["HumanoidRootPart"]["Position"])["Magnitude"] <= 60)) then
											L_178_[2] = L_178_[2] + 1
											if L_178_[2] > 5 then
												L_1_[20] = .1
											...
											(L_1_[48]:RemoteEvent("RegisterAttack")):FireServer(0.125);
				...
				L_182_[4] = function()
					...
					L_183_[4] = {}
					L_183_[2] = L_182_[6]["Character"] and L_182_[6]["Character"]["PrimaryPart"] and L_182_[6]["Character"]["PrimaryPart"]["Position"]
...
local inv = L_1_[7]["Remotes"]["CommF_"]:InvokeServer("getInventory")
if type(inv) == "table" then
	for L_195_forvar0, L_196_forvar1 in pairs(inv) do
		local L_197_ = {}
...
local netMod = L_1_[7]:FindFirstChild("Modules") and L_1_[7]["Modules"]:FindFirstChild("Net")
local customizerRF = netMod and netMod:FindFirstChild("RF/FruitCustomizerRF")
if customizerRF then
	if customizerRF:InvokeServer({["StorageName"] = "Pure Red"; ["Type"] = "AuraSkin"; ["Context"] = "Equip"}) ~= false then Pure_Red_H = true end
	if customizerRF:InvokeServer({["StorageName"] = "Snow White"; ["Type"] = "AuraSkin"; ["Context"] = "Equip"}) ~= false then Snow_White = true end
	if customizerRF:InvokeServer({["StorageName"] = "Snow White"; ["Type"] = "AuraSkin"; ["Context"] = "Equip"}) ~= false then Winter_Sky = true end
	if customizerRF:InvokeServer({["StorageName"] = "Rainbow Saviour"; ["Type"] = "AuraSkin"; ["Context"] = "Equip"}) ~= false then Rainbow_Saviour = true end
end
-- ============================================================
getgenv().UseUnifiedMeleeBuyer = true
getgenv().MeleeV2Manager = {
    State = "IDLE",
    Cooldown = 0,
    Mastered = {},
    Unlocked = {},
    InitialScanDone = false,
    SpeedQuestRunning = false,
    LastBoneRoll = 0,
    LastHopAttempt = 0,
    ...
    Database = {
        ["Superhuman"] = {
            ...
            RequiredMastery = 300,
            BaseStyles = {
                {Name = "Black Leg", Aliases = {"Black Leg", "Dark Step"}, Remote = "BuyBlackLeg", RequiredMastery = 300,
                    CFrames = {Old_World = CFrame.new(-983.62, 12.44, 3990.46), New_World = CFrame.new(-4752.44, 33.92, -4848.04), Three_World = CFrame.new(-5045.61, 370.01, -3182.31)}},
                {Name = "Electro", Aliases = {"Electro", "Electric"}, Remote = "BuyElectro", RequiredMastery = 300,
                    CFrames = {Old_World = CFrame.new(-5382.79, 12.55, -2148.82), New_World = CFrame.new(-4866.16, 33.92, -4767.11), Three_World = CFrame.new(-4996.06, 313.21, -3201.83)}},
                {Name = "Water Kung Fu", Aliases = {"Water Kung Fu", "Fishman Karate"}, Remote = "BuyFishmanKarate", RequiredMastery = 300,
                    CFrames = {Old_World = CFrame.new(61586.96, 19.58, 987.59), New_World = CFrame.new(-4957.68, 35.94, -4665.6), Three_World = CFrame.new(-5023.91, 371.02, -3191.46)}},
                {Name = "Dragon Breath", Aliases = {"Dragon Breath", "Dragon Claw"}, Remote = "BuyDragonBreath", Purchase = function(rem) return rem:InvokeServer("BlackbeardReward", "DragonClaw", "2") end, RequiredMastery = 300,
                    CFrames = {New_World = CFrame.new(-450, 73, 299), Three_World = CFrame.new(-4982, 370, -3209)}}
            },
            CFrames = {New_World = CFrame.new(2940, 67, -3840), Three_World = CFrame.new(-5005, 370, -3199)}
        },
        ["Death Step"] = {
            ...
            Fragments = 5000,
            RequiredMastery = 400,
            MinMasteryToFarmKey = 300,
            BaseStyle = {Name = "Black Leg", Aliases = {"Black Leg", "Dark Step"}, Remote = "BuyBlackLeg", RequiredMastery = 400,
                CFrames = {Old_World = CFrame.new(-983.62, 12.44, 3990.46), New_World = CFrame.new(-4752.44, 33.92, -4848.04), Three_World = CFrame.new(-5045.61, 370.01, -3182.31)}},
            RequiredItem = "Library Key",
            BossName = "Awakened Ice Admiral",
            BossLocation = CFrame.new(5567, 195, -793),
            SeaRequired = "New_World",
            CFrames = {New_World = CFrame.new(6356.47, 296.1, -6762.78), Three_World = CFrame.new(-4999.24, 314.01, -3221.58)}
        },
        ["Sharkman Karate"] = {
            DisplayName = "Sharkman Karate",
            Aliases = {"Sharkman Karate"},
            Remote = "BuySharkmanKarate",
            Beli = 2500000,
            Fragments = 5000,
            RequiredMastery = 400,
            MinMasteryToFarmKey = 300,
            BaseStyle = {Name = "Water Kung Fu", Aliases = {"Water Kung Fu", "Fishman Karate"}, Remote = "BuyFishmanKarate", RequiredMastery = 400,
                CFrames = {Old_World = CFrame.new(61586.96, 19.58, 987.59), New_World = CFrame.new(-4957.68, 35.94, -4665.6), Three_World = CFrame.new(-5023.91, 371.02, -3191.46)}},
            RequiredItem = "Water Key",
            BossName = "Tide Keeper",
            BossLocation = CFrame.new(-3800, 77, -11400),
            SeaRequired = "New_World",
            CFrames = {New_World = CFrame.new(-2599.63, 238.19, -10316), Three_World = CFrame.new(-4971.21, 313.88, -3223.08)}
        },
        ["Electric Claw"] = {
            DisplayName = "Electric Claw",
            Aliases = {"Electric Claw"},
            Remote = "BuyElectricClaw",
            Beli = 3000000,
            Fragments = 5000,
            RequiredMastery = 400,
            BaseStyle = {Name = "Electro", Aliases = {"Electro", "Electric"}, Remote = "BuyElectro", RequiredMastery = 400,
                CFrames = {Old_World = CFrame.new(-5382.79, 12.55, -2148.82), New_World = CFrame.new(-4866.16, 33.92, -4767.11), Three_World = CFrame.new(-4996.06, 313.21, -3201.83)}},
            SpecialQuest = "SpeedQuest30s",
            MansionLocation = CFrame.new(-12548, 337, -7481),
            CFrames = {Three_World = CFrame.new(-10371.48, 330.76, -10131.42)}
        },
        ["Dragon Talon"] = {
            DisplayName = "Dragon Talon",
            Aliases = {"Dragon Talon"},
            Remote = "BuyDragonTalon",
            Beli = 3000000,
            Fragments = 5000,
            RequiredMastery = 400,
            BaseStyle = {Name = "Dragon Breath", Aliases = {"Dragon Breath", "Dragon Claw"}, Remote = "BuyDragonBreath", Purchase = function(rem) return rem:InvokeServer("BlackbeardReward", "DragonClaw", "2") end, RequiredMastery = 400,
                CFrames = {New_World = CFrame.new(-450, 73, 299), Three_World = CFrame.new(-4982, 370, -3209)}},
            RequiredItem = "Fire Essence",
            BonesLocation = CFrame.new(-9515, 142, 5535),
            CFrames = {Three_World = CFrame.new(5661.89, 1210.87, 863.17)}
        },
        ["Godhuman"] = {
            DisplayName = "Godhuman",
            Aliases = {"Godhuman", "God Human"},
            Remote = "BuyGodhuman",
            Beli = 5000000,
            Fragments = 5000,
            RequiredMastery = 400,
            CFrames = {Three_World = CFrame.new(-13774.1, 333.73, -9879.91)}
        }
    },

    ScanServerUnlocks = function(self)
        if self.InitialScanDone then return end
        self.InitialScanDone = true
        
        -- In Sea 3 (Level >= 1500), all V1 styles & Superhuman are unlocked by default
        if Three_World or (L_1_[24] and L_1_[24].Value >= 1500) then
            self.Unlocked["Superhuman"] = true
            self.Mastered["Superhuman"] = 400
            self.Unlocked["Black Leg"] = true
            self.Mastered["Black Leg"] = 400
            self.Unlocked["Electro"] = true
            self.Mastered["Electro"] = 400
            self.Unlocked["Water Kung Fu"] = true
            self.Mastered["Water Kung Fu"] = 400
            self.Unlocked["Dragon Breath"] = true
            self.Mastered["Dragon Breath"] = 400
        end
        
        -- Scan Backpack and Character for any owned tools
        local lp = L_1_[35]
        ...
        local function scanTools(container)
            ...
            for _, t in pairs(container:GetChildren()) do
                if t:IsA("Tool") then
                    local lvl = t:FindFirstChild("Level") and t.Level.Value or 400
                    self.Mastered[t.Name] = math.max(self.Mastered[t.Name] or 0, lvl)
                    self.Unlocked[t.Name] = true
                    for dbName, dbData in pairs(self.Database) do
                        if t.Name == dbName or (dbData.Aliases and table.find(dbData.Aliases, t.Name)) then
                            self.Unlocked[dbName] = true
                            self.Mastered[dbName] = math.max(self.Mastered[dbName] or 0, lvl)
                        end
                    end
                end
            end
    ...
    -- 1. Helper functions
    GetCurrentlyEquipped = function(self)
        local lp = L_1_[35]
        if not lp then return nil, 0, nil end
        local char = lp.Character
        local bp = lp:FindFirstChild("Backpack")
        
        for _, v2Data in pairs(self.Database) do
            for _, alias in ipairs(v2Data.Aliases or {}) do
                local tool = (char and char:FindFirstChild(alias)) or (bp and bp:FindFirstChild(alias))
                if tool and tool:IsA("Tool") and tostring(tool.ToolTip) == "Melee" then
                    local lvl = tool:FindFirstChild("Level") and tool.Level.Value or 0
                    return v2Data.DisplayName, lvl, tool
                end
            end
            if v2Data.BaseStyles then
                for _, bs in ipairs(v2Data.BaseStyles) do
                    for _, alias in ipairs(bs.Aliases or {}) do
                        local tool = (char and char:FindFirstChild(alias)) or (bp and bp:FindFirstChild(alias))
                        if tool and tool:IsA("Tool") and tostring(tool.ToolTip) == "Melee" then
                            local lvl = tool:FindFirstChild("Level") and tool.Level.Value or 0
                            return bs.Name, lvl, tool
                        end
                    end
                end
            elseif v2Data.BaseStyle then
                for _, alias in ipairs(v2Data.BaseStyle.Aliases or {}) do
                    local tool = (char and char:FindFirstChild(alias)) or (bp and bp:FindFirstChild(alias))
                    if tool and tool:IsA("Tool") and tostring(tool.ToolTip) == "Melee" then
                        local lvl = tool:FindFirstChild("Level") and tool.Level.Value or 0
                        return v2Data.BaseStyle.Name, lvl, tool
                    end
                end
            end
        end
    ...
    HasItem = function(self, itemName)
        ...
        local bp = lp:FindFirstChild("Backpack")
        local char = lp.Character
        if (bp and L_1_[45]["ffc"](bp, itemName)) or (char and L_1_[45]["ffc"](char, itemName)) then
            return true
        end
        return false
    end,

    GetTargetCFrame = function(self, cframes)
        if not cframes then return nil end
        if Old_World and cframes.Old_World then return cframes.Old_World end
        if New_World and cframes.New_World then return cframes.New_World end
        if Three_World and cframes.Three_World then return cframes.Three_World end
        return nil
    end,

    -- 2. State Machine Evaluator
    GetTarget = function(self)
        if os.clock() < self.Cooldown then return nil end
        if Quest == "World 2" or Quest == "TravelZou" then return nil end
        
        self:ScanServerUnlocks()
        
        local currentMelee, currentMastery, currentTool = self:GetCurrentlyEquipped()
        local lp = L_1_[35]
        local beli = (lp and lp.Data and lp.Data.Beli and lp.Data.Beli.Value) or 0
        local frags = (lp and lp.Data and lp.Data:FindFirstChild("Fragments") and lp.Data.Fragments.Value) or 0
        
        if currentMelee and currentMastery then
            self.Mastered[currentMelee] = math.max(self.Mastered[currentMelee] or 0, currentMastery)
            if currentMastery >= 300 then
                self.Unlocked[currentMelee] = true
            end
        end
        
        -- If current melee is under 400 mastery, stick to it and level it up without switching teachers!
        if currentMelee and currentMastery and currentMastery < 400 then
            getgenv().SetTask("MiniTask", "Melee | Leveling " .. currentMelee .. " (" .. currentMastery .. "/400)")
            return nil
        end
        
        local cfg = getgenv().Config and getgenv().Config.Melee
        
        -- TOP PRIORITY: AUTO RAID FOR 5K FRAGMENTS (In Sea 2 / Sea 3)
        if (New_World or Three_World) and frags < 5000 and (L_1_[24] and L_1_[24].Value >= 1100) then
            if not _G.RaidNeedBeli then
                getgenv().SetTask("MiniTask", "Auto Raid | Farming Fragments (" .. frags .. "/5,000 F)")
                return {Action = "AUTO_RAID", TargetFrags = 5000, Reason = "Auto Raid 5K F"}
            end
        end
        
        -- PHASE 1 & 2: V1 BASE STYLES & SUPERHUMAN (Sea 1 / Sea 2 only if Superhuman not unlocked)
        if not self.Unlocked["Superhuman"] and not Three_World and (cfg == nil or cfg["Super Huamn"] ~= false or cfg["GodHuman"] ~= false) then
            local superhumanData = self.Database["Superhuman"]
            local needBaseEquip = nil
            
            for _, bs in ipairs(superhumanData.BaseStyles) do
                local targetCF = self:GetTargetCFrame(bs.CFrames)
                local bsMastery = self.Mastered[bs.Name] or (currentMelee == bs.Name and currentMastery) or 0
                
                if bsMastery < 300 and targetCF then
                    if currentMelee ~= bs.Name then
                        needBaseEquip = {Action = "EQUIP_BASE", Name = bs.Name, Remote = bs.Remote, Purchase = bs.Purchase, CFrame = targetCF}
                        break
                    else
                        getgenv().SetTask("MiniTask", "Melee V1 | " .. bs.Name .. " (" .. currentMastery .. "/300)")
                        return nil
                    end
                end
            end
            
            if needBaseEquip then
                return needBaseEquip
            end
            
            local shTargetCF = self:GetTargetCFrame(superhumanData.CFrames)
            if shTargetCF then
                if beli >= superhumanData.Beli then
                    return {Action = "UNLOCK_V2", Name = "Superhuman", Remote = superhumanData.Remote, CFrame = shTargetCF}
                else
                    getgenv().SetTask("MiniTask", "Melee V2 | Superhuman [Beli: " .. math.floor(beli/1000) .. "K/3,000K]")
                    return nil
                end
            end
        end
        
        -- PHASE 3: SEA 2 V2 STYLES (Death Step & Sharkman Karate)
        local v2List = {"Death Step", "Sharkman Karate"}
        for _, v2Name in ipairs(v2List) do
            local v2Data = self.Database[v2Name]
            local isEnabled = (cfg == nil or cfg[v2Name] ~= false or cfg["GodHuman"] ~= false)
            
            if isEnabled and not self.Unlocked[v2Name] then
                if v2Data.RequiredItem and not self:HasItem(v2Data.RequiredItem) and New_World then
                    return {Action = "FARM_BOSS_KEY", Item = v2Data.RequiredItem, BossName = v2Data.BossName, BossLocation = v2Data.BossLocation}
                end
                
                local v2TargetCF = self:GetTargetCFrame(v2Data.CFrames)
                if v2TargetCF then
                    if beli < v2Data.Beli then
                        getgenv().SetTask("MiniTask", "Melee V2 | " .. v2Name .. " [Need $" .. math.floor(v2Data.Beli/1000) .. "K Beli] ($" .. math.floor(beli/1000) .. "K/$" .. math.floor(v2Data.Beli/1000) .. "K)")
                        return nil
                    end
                    return {Action = "UNLOCK_V2", Name = v2Name, Remote = v2Data.Remote, CFrame = v2TargetCF}
                end
            end
        end
        
        -- PRIORITY 5: Sea 3 V2 Styles (Electric Claw & Dragon Talon)
        local v3List = {"Electric Claw", "Dragon Talon"}
        for _, v2Name in ipairs(v3List) do
            local v2Data = self.Database[v2Name]
            local isEnabled = (cfg == nil or cfg[v2Name] ~= false or cfg["GodHuman"] ~= false)
            
            if isEnabled and not self.Unlocked[v2Name] then
                if v2Data.RequiredItem and not self:HasItem(v2Data.RequiredItem) and v2Data.RequiredItem == "Fire Essence" and Three_World then
                    return {Action = "FARM_ITEM", Item = "Fire Essence", Location = v2Data.BonesLocation}
                end
                ...
                if v2Data.SpecialQuest == "SpeedQuest30s" and Three_World then
                    local v2TargetCF = self:GetTargetCFrame(v2Data.CFrames)
                    return {Action = "SPEED_QUEST", Name = "Electric Claw", CFrame = v2TargetCF, Mansion = v2Data.MansionLocation}
                end
        ...
        -- If in Sea 2 and both Sea 2 V2 styles (Death Step & Sharkman Karate) are completely unlocked: Return to Sea 3!
        if New_World and self.Unlocked["Death Step"] and self.Unlocked["Sharkman Karate"] and (L_1_[24] and L_1_[24].Value >= 1500) then
            getgenv().SetTask("MiniTask", "Melee V2 | Both Sea 2 Melees Unlocked! Returning to Sea 3 (Zou)...")
            return {Action = "TRAVEL_SEA", Remote = "TravelZou", Destination = "Sea 3"}
        end
        
        -- PHASE 4: GODHUMAN
        local godData = self.Database["Godhuman"]
        local godTargetCF = self:GetTargetCFrame(godData.CFrames)
        if godTargetCF and (cfg == nil or cfg["GodHuman"] ~= false) then
            if not self.Unlocked["Godhuman"] and currentMelee ~= "Godhuman" then
                if beli >= godData.Beli and frags >= godData.Fragments then
                    return {Action = "UNLOCK_V2", Name = "Godhuman", Remote = godData.Remote, CFrame = godTargetCF}
                else
                    getgenv().SetTask("MiniTask", "Melee V2 | Godhuman [" .. math.floor(beli/1000) .. "K/5,000K | " .. frags .. "/5,000 F]")
                    return nil
                end
            elseif currentMelee == "Godhuman" then
                getgenv().SetTask("MiniTask", "Melee V2 | Godhuman Mastery (" .. currentMastery .. ")")
                return nil
            end
        end
    ...
    Execute = function(self, goal)
        if not goal then return end
        
        if goal.Action == "AUTO_RAID" then
            L_1_[45]["RunRaidPipeline"](goal.Reason)
            return
        end
        
        if goal.Action == "TRAVEL_SEA" then
            getgenv().SetTask("MiniTask", "Melee V2 | Teleporting to " .. (goal.Destination or "Next Sea") .. "...")
            local rem = L_1_[7].Remotes.CommF_
            pcall(function()
                rem:InvokeServer(goal.Remote)
            end)
            task.wait(3)
            self.Cooldown = os.clock() + 10
            return
            
        elseif goal.Action == "FARM_BOSS_KEY" then
            local boss = nil
            if L_1_[40] then boss = L_1_[40]:FindFirstChild(goal.BossName) end
            if not boss and L_1_[7] then boss = L_1_[7]:FindFirstChild(goal.BossName) end
            
            -- If we now have the required key in Backpack/Character, immediately buy V2 style!
            if self:HasItem(goal.Item) then
                if goal.Item == "Library Key" then
                    getgenv().SetTask("MiniTask", "Melee V2 | Library Key acquired! Unlocking Death Step...")
                    L_1_[31](CFrame.new(6356.47, 296.1, -6762.78), 1.5)
                    task.wait(1)
                    L_1_[7].Remotes.CommF_:InvokeServer("BuyDeathStep")
                    task.wait(0.2)
                    L_1_[7].Remotes.CommF_:InvokeServer("BuyDeathStep", true)
                    self.Unlocked["Death Step"] = true
                    self.Cooldown = os.clock() + 5
                    return
                elseif goal.Item == "Water Key" then
                    getgenv().SetTask("MiniTask", "Melee V2 | Water Key acquired! Unlocking Sharkman Karate...")
                    L_1_[31](CFrame.new(-2599.63, 238.19, -10316), 1.5)
                    task.wait(1)
                    L_1_[7].Remotes.CommF_:InvokeServer("BuySharkmanKarate")
                    task.wait(0.2)
                    L_1_[7].Remotes.CommF_:InvokeServer("BuySharkmanKarate", true)
                    self.Unlocked["Sharkman Karate"] = true
                    self.Cooldown = os.clock() + 5
                    return
                end
            end
            
            if boss and boss:FindFirstChild("HumanoidRootPart") and L_1_[45]["gh"](boss) > 0 then
                getgenv().SetTask("MiniTask", "Melee V2 | Defeating " .. goal.BossName .. " [HP: " .. math.floor(L_1_[45]["gh"](boss)) .. "]")
                L_1_[31](boss.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0), 1.5)
                L_1_[14]()
                if not L_1_[45]["ffc"](L_1_[35]["Character"], "HasBuso") then
                    L_1_[7]["Remotes"]["CommF_"]:InvokeServer("Buso")
                end
                ...
                getgenv().SetTask("MiniTask", "Melee V2 | " .. goal.BossName .. " not spawned | Hopping server...")
                L_1_[31](goal.BossLocation, 1.5)
                if os.clock() - self.LastHopAttempt > 8 then
                    self.LastHopAttempt = os.clock()
                    pcall(function()
        ...
        elseif goal.Action == "EQUIP_BASE" or goal.Action == "UNLOCK_V2" then
            getgenv().SetTask("MiniTask", "Melee V2 | Going to " .. goal.Name .. " trainer")
            local started = tick()
            repeat
                local character = L_1_[35].Character
                local root = character and character:FindFirstChild("HumanoidRootPart")
                if root and (root.Position - goal.CFrame.Position).Magnitude <= 12 then break end
                L_1_[31](goal.CFrame, 1.5)
                L_1_[45]["wt"]()
            until not getgenv().AutoFarm or tick() - started >= 60

            local character = L_1_[35].Character
            local root = character and character:FindFirstChild("HumanoidRootPart")
            if not root or (root.Position - goal.CFrame.Position).Magnitude > 25 then
                self.Cooldown = os.clock() + 5
                return
            end

            getgenv().SetTask("MiniTask", "Melee V2 | Talking to " .. goal.Name .. " trainer")
            pcall(function()
                local rem = L_1_[7].Remotes.CommF_
                if goal.Purchase then
                    goal.Purchase(rem)
                else
                    rem:InvokeServer(goal.Remote)
                    task.wait(0.2)
                    rem:InvokeServer(goal.Remote, true)
                end
            end)
            
            L_1_[45]["wt"](1)
            local currentMelee = self:GetCurrentlyEquipped()
            if currentMelee == goal.Name then
                self.Unlocked[goal.Name] = true
                getgenv().SetTask("MiniTask", "Melee V2 | Equipped " .. goal.Name .. " ✅")
                self.Cooldown = os.clock() + 3
            else
                self.Cooldown = os.clock() + 5
            end
            
        elseif goal.Action == "SPEED_QUEST" then
            local rem = L_1_[7].Remotes.CommF_
            local root = L_1_[35].Character and L_1_[35].Character:FindFirstChild("HumanoidRootPart")
            
            getgenv().SetTask("MiniTask", "Melee V2 | Going to Previous Hero...")
            L_1_[31](goal.CFrame, 1.5)
            task.wait(0.5)
            
            getgenv().SetTask("MiniTask", "Melee V2 | Starting Electric Claw Quest...")
            rem:InvokeServer("BuyElectricClaw", "Start")
            task.wait(0.3)
            
            getgenv().SetTask("MiniTask", "Melee V2 | Racing to Mansion in 30s...")
            local startRace = os.clock()
            while os.clock() - startRace < 20 do
                L_1_[45]["wt"]()
                local curRoot = L_1_[35].Character and L_1_[35].Character:FindFirstChild("HumanoidRootPart")
                if curRoot then
                    L_1_[31](goal.Mansion, 1.5)
                    if (goal.Mansion.Position - curRoot.Position).Magnitude <= 30 then
                        break
                    end
                end
            end
            
            getgenv().SetTask("MiniTask", "Melee V2 | Returning to Previous Hero...")
            local startReturn = os.clock()
            while os.clock() - startReturn < 20 do
                L_1_[45]["wt"]()
                local curRoot = L_1_[35].Character and L_1_[35].Character:FindFirstChild("HumanoidRootPart")
                if curRoot then
                    L_1_[31](goal.CFrame, 1.5)
                    if (goal.CFrame.Position - curRoot.Position).Magnitude <= 30 then
                        break
                    end
                end
            end
            
            getgenv().SetTask("MiniTask", "Melee V2 | Purchasing Electric Claw...")
            rem:InvokeServer("BuyElectricClaw")
            rem:InvokeServer("BuyElectricClaw", true)
            task.wait(1)
            
            local currentMelee = self:GetCurrentlyEquipped()
            if currentMelee == "Electric Claw" then
                self.Unlocked["Electric Claw"] = true
                getgenv().SetTask("MiniTask", "Melee V2 | Electric Claw Unlocked! ✅")
            end
            self.Cooldown = os.clock() + 5
            
        elseif goal.Action == "FARM_ITEM" then
            if goal.Item == "Fire Essence" and Three_World then
                if os.clock() - self.LastBoneRoll > 5 then
                    self.LastBoneRoll = os.clock()
                    local rem = L_1_[7].Remotes.CommF_
                    rem:InvokeServer("Bones", "Buy", 1, 1)
                end
            end
        end
    end
}
getgenv().Sea1MeleeManager = getgenv().MeleeV2Manager

L_1_[39] = function()
	local L_198_ = {}
	
	-- PRIORITY 0: Auto Grab Spawned Fruits on Map (Highest Priority)
	local spawnedFruit, fruitHandle = L_1_[45]["FindSpawnedFruit"]()
	if spawnedFruit and fruitHandle then
		getgenv().SetTask("MiniTask", "Fruit Finder | Collecting " .. spawnedFruit.Name .. " 🍎")
		L_1_[45]["Status"](" Status : Collecting " .. spawnedFruit.Name)
		local started = tick()
		repeat
			L_1_[45]["wt"]()
			local char = L_1_[35].Character
			local root = char and char:FindFirstChild("HumanoidRootPart")
			if root and fruitHandle and fruitHandle.Parent then
				L_1_[31](fruitHandle.CFrame, 1.5)
		...
		until not spawnedFruit.Parent or not fruitHandle.Parent or (tick() - started >= 30) or not getgenv().AutoFarm
		
		task.wait(0.5)
		pcall(function()
			for _, item in pairs(L_1_[35].Backpack:GetChildren()) do
				if item:IsA("Tool") and (item.ToolTip == "Blox Fruit" or string.find(item.Name, "Fruit") or string.find(item.Name, "-")) then
					local orig = item:GetAttribute("OriginalName") or string.gsub(item.Name, " Fruit", "")
					L_1_[7].Remotes.CommF_:InvokeServer("StoreFruit", orig, item)
				end
			end
		end)
		return
	...
	if Three_World then
		pcall(function()
			local cpRes = tostring(L_1_[7]["Remotes"]["CommF_"]:InvokeServer("CakePrinceSpawner"))
			if cpRes and (string.match(cpRes, "%d+") == nil or cpRes == "nil") then
	...
	if Three_World then
		if L_1_[45]["ffc"](L_1_[40], "Cake Prince") or L_1_[45]["ffc"](L_1_[7], "Cake Prince") then
			if getgenv().SetTask then
				getgenv().SetTask("MiniTask", "Boss | Defeating Cake Prince")
			end
			if L_1_[45]["ffc"](L_1_[40], "Cake Prince") then
				for L_199_forvar0, L_200_forvar1 in pairs(L_1_[40]:GetChildren()) do
					...
					if L_201_[3]["Name"] == "Cake Prince" and L_1_[45]["gh"](L_201_[3]) > 0 then
						repeat
							L_1_[45]["wt"]()
							if not L_1_[45]["ffc"](L_1_[35]["Character"], "HasBuso") then
								L_1_[35]["Character"]["HumanoidRootPart"]["Remotes"]["CommF_"]:InvokeServer("Buso")
							...
							L_1_[14]()
						until not L_201_[3]["Parent"] or L_1_[45]["gh"](L_201_[3]) <= 0 or not(getgenv())["AutoFarm"]
					end
				end
			elseif L_1_[45]["ffc"](L_1_[7], "Cake Prince") then
				for L_202_forvar0, L_203_forvar1 in pairs(L_1_[7]:GetChildren()) do
					...
					if L_204_[3]["Name"] == "Cake Prince" and L_1_[45]["gh"](L_204_[3]) > 0 then
						repeat
							L_1_[45]["wt"]()
							if not L_1_[45]["ffc"](L_1_[35]["Character"], "HasBuso") then
								L_1_[35]["Character"]["HumanoidRootPart"]["Remotes"]["CommF_"]:InvokeServer("Buso")
							...
							L_1_[14]()
						until not L_204_[3]["Parent"] or L_1_[45]["gh"](L_204_[3]) <= 0 or not(getgenv())["AutoFarm"]
					end
				end
			end
		end
	...
	if Three_World then
		if L_1_[40]:FindFirstChild("rip_indra True Form") or L_1_[7]:FindFirstChild("rip_indra True Form") then
			local tProg = L_1_[7]["Remotes"]["CommF_"]:InvokeServer("TushitaProgress")
			if type(tProg) == "table" and not tProg["OpenedDoor"] then
				local L_205_ = {}
				L_205_[1] = tProg
				if not L_205_[1]["OpenedDoor"] then
					if L_1_[35]["Backpack"]:FindFirstChild("Holy Torch") or L_1_[35]["Character"]:FindFirstChild("Holy Torch") then
						if getgenv().SetTask then
							getgenv().SetTask("MiniTask", "Tushita | Lighting 5 Torches")
						end
				...
				if L_1_[40]:FindFirstChild("rip_indra True Form") or L_1_[7]:FindFirstChild("rip_indra True Form") then
					if L_1_[40]:FindFirstChild("rip_indra True Form") then
						for L_212_forvar0, L_213_forvar1 in pairs(L_1_[40]:GetChildren()) do
							...
							if L_214_[1]["Name"] == "rip_indra True Form" and L_1_[45]["gh"](L_214_[1]) > 0 then
								repeat
									L_1_[45]["wt"](.1)
									if not L_1_[45]["ffc"](L_1_[35]["Character"], "HasBuso") then
										L_1_[35]["Character"]["HumanoidRootPart"]["Remotes"]["CommF_"]:InvokeServer("Buso")
									...
									L_1_[14]()
								until not L_214_[1]["Parent"] or L_1_[45]["gh"](L_214_[1]) <= 0
							end
						end
		...
		if L_1_[40]:FindFirstChild("rip_indra True Form") or L_1_[7]:FindFirstChild("rip_indra True Form") then
			if L_1_[40]:FindFirstChild("rip_indra True Form") then
				for L_233_forvar0, L_234_forvar1 in pairs(L_1_[40]:GetChildren()) do
					...
					if L_235_[3]["Name"] == "rip_indra True Form" and L_1_[45]["gh"](L_235_[3]) > 0 then
						repeat
							L_1_[45]["wt"](.1)
							if not L_1_[45]["ffc"](L_1_[35]["Character"], "HasBuso") then
								L_1_[35]["Character"]["HumanoidRootPart"]["Remotes"]["CommF_"]:InvokeServer("Buso")
							...
							L_1_[14]()
						until not L_235_[3]["Parent"] or L_1_[45]["gh"](L_235_[3]) <= 0
					end
				end
			elseif L_1_[7]:FindFirstChild("rip_indra True Form") then
				L_1_[31]((L_1_[7]:FindFirstChild("rip_indra True Form"))["HumanoidRootPart"]["CFrame"] * CFrame["new"](0, 30, 0))
	...
	if Three_World and not Mirror_Fractal_H then
		if L_1_[45]["ffc"](L_1_[35]["Backpack"], "Sweet Chalice") or L_1_[45]["ffc"](L_1_[35]["Character"], "Sweet Chalice") or L_1_[45]["ffc"](L_1_[35]["Backpack"], "God's Chalice") or L_1_[45]["ffc"](L_1_[35]["Character"], "God's Chalice") or L_1_[45]["ffc"](L_1_[40], "Dough King") or L_1_[45]["ffc"](L_1_[7], "Dough King") then
			repeat
				...
				pcall(function() inv = L_1_[7]["Remotes"]["CommF_"]:InvokeServer("getInventory") end)
				if type(inv) == "table" then
					for L_236_forvar0, L_237_forvar1 in pairs(inv) do
						local L_238_ = {}
						L_238_[2], L_238_[3] = L_236_forvar0, L_237_forvar1
						if type(L_238_[3]) == "table" and L_238_[3]["Type"] == "Material" then
				...
				if L_1_[45]["ffc"](L_1_[40], "Dough King") or L_1_[45]["ffc"](L_1_[7], "Dough King") then
					if getgenv().SetTask then
						getgenv().SetTask("MiniTask", "Boss | Defeating Dough King")
					end
					if L_1_[45]["ffc"](L_1_[40], "Dough King") then
						for L_239_forvar0, L_240_forvar1 in pairs(L_1_[40]:GetChildren()) do
							...
							if L_241_[1]["Name"] == "Dough King" and L_1_[45]["gh"](L_241_[1]) > 0 then
								repeat
									wait(.1)
									L_1_[31](L_241_[1]["HumanoidRootPart"]["CFrame"] * CFrame["new"](0, -30, 0), 1.5)
									L_1_[14]()
								until not L_241_[1]["Parent"] or L_1_[45]["gh"](L_241_[1]) <= 0
								local doughInv = (game:GetService("ReplicatedStorage"))["Remotes"]["CommF_"]:InvokeServer("getInventory")
								if type(doughInv) == "table" then
									for L_242_forvar0, L_243_forvar1 in pairs(doughInv) do
										local L_244_ = {}
				...
				elseif L_1_[45]["ffc"](L_1_[35]["Backpack"], "Sweet Chalice") or L_1_[45]["ffc"](L_1_[35]["Character"], "Sweet Chalice") and not Mirror_Fractal_H then
					...
					elseif (L_1_[35]["Character"]["HumanoidRootPart"]["Position"] - Vector3["new"](-2286.6843261719, 146.56562805176, -12226.881835938))["Magnitude"] < 1800 then
						...
						if Monster ~= nil and L_1_[45]["gh"](Monster) > 0 then
							local L_247_ = {}
							...
							repeat
								...
								L_1_[14]()
							until not Monster["Parent"] or L_1_[45]["gh"](Monster) <= 0
							StatrMagnet = false
	...
	if Three_World then
		for _, bossName in ipairs({"Diablo", "Deandre", "Urban"}) do
			local boss = L_1_[40]:FindFirstChild(bossName) or L_1_[7]:FindFirstChild(bossName)
			if boss and boss:FindFirstChild("HumanoidRootPart") and boss:FindFirstChildOfClass("Humanoid") and boss:FindFirstChildOfClass("Humanoid").Health > 0 then
				getgenv().SetTask("MiniTask", "Elite Hunter | Defeating " .. bossName)
				
				-- 1. Ensure Elite Hunter Quest is accepted once if not active
				local pgui = L_1_[35]:FindFirstChild("PlayerGui")
				local mainGui = pgui and pgui:FindFirstChild("Main")
				local questGui = mainGui and mainGui:FindFirstChild("Quest")
				local hasQuest = questGui and questGui.Visible and questGui:FindFirstChild("Container") and questGui.Container:FindFirstChild("QuestTitle") and questGui.Container.QuestTitle:FindFirstChild("Title") and string.find(tostring(questGui.Container.QuestTitle.Title.Text), bossName)
				
				if not hasQuest then
					pcall(function() L_1_[7].Remotes.CommF_:InvokeServer("EliteHunter") end)
					task.wait(0.3)
				end
				...
				repeat
					...
					local char = L_1_[35].Character
					local hum = char and char:FindFirstChildOfClass("Humanoid")
					local bp = L_1_[35]:FindFirstChild("Backpack")
					
					-- Equip tool directly if not equipped
					if char and not char:FindFirstChildOfClass("Tool") and bp and hum then
						for _, t in pairs(bp:GetChildren()) do
							if t:IsA("Tool") and (t.ToolTip == "Melee" or t.ToolTip == "Sword" or t.Name == "Electro" or t.Name == "Superhuman") then
								hum:EquipTool(t)
								break
							end
						end
					...
					-- Position 20 studs above boss (within weapon reach)
					local bRoot = boss:FindFirstChild("HumanoidRootPart")
					if bRoot then
						L_1_[31](bRoot.CFrame * CFrame.new(0, 20, 0), 1.5)
					end
					...
					-- Attack with tool activate and click
				...
				until not boss.Parent or not boss:FindFirstChildOfClass("Humanoid") or boss:FindFirstChildOfClass("Humanoid").Health <= 0 or not getgenv().AutoFarm
			end
		end
	end

	-- INTERCEPT: Sea 1 Melee Manager
	if getgenv().Sea1MeleeManager then
	    local meleeTarget, trainerCFrame = getgenv().Sea1MeleeManager:GetTarget()
	    if meleeTarget then
	        getgenv().Sea1MeleeManager:Execute(meleeTarget, trainerCFrame)
	        return -- yield control for this frame so we don't conflict with LevelFarm
	    end
	end
	
	-- INTERCEPT: Auto Raid (5K Fragments)
	local autoRaidCfg = (getgenv().Config and getgenv().Config.Settings and (getgenv().Config.Settings["Auto Raid (5K F)"] or getgenv().Config.Settings["Auto Raid"]))
	if autoRaidCfg == nil and getgenv().Configs then
		autoRaidCfg = getgenv().Configs["Auto Raid (5K F)"]
	end
	
	local currentBeli = (L_1_[35] and L_1_[35].Data and L_1_[35].Data.Beli and L_1_[35].Data.Beli.Value) or 0
	local bp = L_1_[35] and L_1_[35]:FindFirstChild("Backpack")
	local ch = L_1_[35] and L_1_[35].Character
	local hasRaidTool = (bp and bp:FindFirstChild("Special Microchip")) or (ch and ch:FindFirstChild("Special Microchip"))
	if not hasRaidTool and bp then
		for _, t in pairs(bp:GetChildren()) do
			if t:IsA("Tool") and (t.ToolTip == "Blox Fruit" or string.find(t.Name, "Fruit") or LowValueRaidFruitLookup[t.Name]) then
				hasRaidTool = true
				break
			end
		end
	...
	if autoRaidCfg and (New_World or Three_World) and (L_1_[17] and L_1_[17].Value < 5000) and (L_1_[24] and L_1_[24].Value >= 1100) then
		if hasRaidTool or currentBeli >= 100000 or not _G.RaidNeedBeli then
			L_1_[45]["RunRaidPipeline"]("Auto Raid 5K F")
			return
		end
	end
	...
	-- LEVEL SKIP SPEEDRUN (Sea 1: Level 10 -> 80 directly on Sky Bandits)
	local currentLvl = L_1_[24]["Value"]
	if Old_World and currentLvl >= 10 and currentLvl < 80 then
		local banditPos = CFrame.new(-4965, 357, -2848)
		local char = L_1_[35].Character
		local root = char and char:FindFirstChild("HumanoidRootPart")
		
		getgenv().SetTask("MiniTask", "Level Skip | Sky Bandit | Level " .. currentLvl .. "/80")
		
		if root and (banditPos.Position - root.Position).Magnitude > 600 then
			L_1_[31](banditPos, 1.5)
			return
		end
		
		local targetEnemy = nil
		for _, enemy in pairs(L_1_[40]:GetChildren()) do
			local hum = enemy:FindFirstChildOfClass("Humanoid")
			local root = enemy:FindFirstChild("HumanoidRootPart")
			if enemy.Name == "Sky Bandit" and hum and root and hum.Health > 0 then
				targetEnemy = enemy
				break
			end
		end
		...
		if targetEnemy then
			_G.WaitMobTick = nil
			local mobDeadline = tick() + 20
			repeat
				L_1_[45]["wt"]()
				if targetEnemy:FindFirstChild("HumanoidRootPart") then
					L_1_[45]["BN"]("Sky Bandit")
					L_1_[31](targetEnemy.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0), 1.5)
					if not L_1_[45]["ffc"](char, "HasBuso") then
						L_1_[7].Remotes.CommF_:InvokeServer("Buso")
					end
					L_1_[14]()
				end
			until not targetEnemy.Parent or not targetEnemy:FindFirstChildOfClass("Humanoid") or targetEnemy.Humanoid.Health <= 0 or not getgenv().AutoFarm or tick() >= mobDeadline or L_1_[24]["Value"] >= 80
			return
		else
			L_1_[31](banditPos, 1.5)
			_G.WaitMobTick = _G.WaitMobTick or tick()
			if tick() - _G.WaitMobTick > 15 then
				local meleeManager = getgenv().Sea1MeleeManager
				if not meleeManager or os.clock() >= (meleeManager.SuppressHopUntil or 0) then
					L_1_[45]["HopLowServer"](3)
				end
				_G.WaitMobTick = nil
			end
	...
	if L_1_[24]["Value"] < 2650 or not Three_World then
		-- UNIFIED SEA 1 & 2 QUEST LOOP
		local char = L_1_[35].Character
		local root = char and char:FindFirstChild("HumanoidRootPart")
		local hum = char and char:FindFirstChildOfClass("Humanoid")
		local pgui = L_1_[35]:FindFirstChild("PlayerGui")
		local questGui = pgui and pgui:FindFirstChild("Main") and pgui.Main:FindFirstChild("Quest")
		local qVisible = questGui and questGui.Visible
		local qTitle = qVisible and questGui:FindFirstChild("Container") and questGui.Container:FindFirstChild("QuestTitle") and questGui.Container.QuestTitle:FindFirstChild("Title") and tostring(questGui.Container.QuestTitle.Title.Text) or ""

		local isCorrectQuest = qVisible and NameEnemy and L_1_[45]["sf"](qTitle, tostring(NameEnemy))

		if isCorrectQuest then
			-- 1. Farm the assigned regular mob
			local targetEnemy = nil
			for _, enemy in pairs(L_1_[40]:GetChildren()) do
				local eHum = enemy:FindFirstChildOfClass("Humanoid")
				local eRoot = enemy:FindFirstChild("HumanoidRootPart")
				if enemy.Name == Enemy and eHum and eRoot and eHum.Health > 0 then
					targetEnemy = enemy
					break
				end
			end
			...
			if targetEnemy then
				...
				repeat
					L_1_[45]["wt"]()
					local curRoot = targetEnemy:FindFirstChild("HumanoidRootPart")
					local curHum = targetEnemy:FindFirstChildOfClass("Humanoid")
					if curRoot and curHum and curHum.Health > 0 then
						L_1_[45]["BN"](targetEnemy.Name)
						L_1_[31](curRoot.CFrame * CFrame.new(0, 25, 0), 1.5)
						if not L_1_[45]["ffc"](L_1_[35].Character, "HasBuso") then
							L_1_[7].Remotes.CommF_:InvokeServer("Buso")
						end
						L_1_[14]()
					end
				until not targetEnemy.Parent or not targetEnemy:FindFirstChildOfClass("Humanoid") or targetEnemy.Humanoid.Health <= 0 or not (L_1_[35].PlayerGui.Main:FindFirstChild("Quest") and L_1_[35].PlayerGui.Main.Quest.Visible) or not getgenv().AutoFarm or Quest ~= nil
			else
				L_1_[31](EnemyPos, 1.5)
				_G.WaitMobTick = _G.WaitMobTick or tick()
				if tick() - _G.WaitMobTick > 25 then
					local meleeManager = getgenv().Sea1MeleeManager
					if not meleeManager or os.clock() >= (meleeManager.SuppressHopUntil or 0) then
						L_1_[45]["HopLowServer"](3)
					end
			...
			-- 2. No quest or unwanted quest: fly to QuestPos and overwrite with correct Level Quest!
			if QuestPos and QuestName and QuestNumber and root and hum and hum.Health > 0 then
				local dist = (QuestPos.Position - root.Position).Magnitude
				if dist > 15 then
					getgenv().SetTask("MiniTask", "Level Farm | Moving to " .. QuestName .. " (" .. math.floor(dist) .. " studs)...")
					L_1_[31](QuestPos, 1.5)
				else
					getgenv().SetTask("MiniTask", "Level Farm | Taking Quest " .. QuestName .. " (" .. QuestNumber .. ")...")
					L_1_[7].Remotes.CommF_:InvokeServer("StartQuest", QuestName, QuestNumber)
					task.wait(0.3)
				end
			end
		end
		...
		if L_1_[45]["ffc"](L_1_[7], "Cake Prince") then
			for L_278_forvar0, L_279_forvar1 in pairs(L_1_[7]:GetChildren()) do
				...
				if L_280_[2]["Name"] == "Cake Prince" and (L_280_[2]:FindFirstChildOfClass("Humanoid") and (L_280_[2]:FindFirstChildOfClass("Humanoid"))["Health"] > 0) then
		...
		elseif L_1_[45]["ffc"](L_1_[40], "Cake Prince") then
			for L_281_forvar0, L_282_forvar1 in pairs(L_1_[40]:GetChildren()) do
				...
				if L_283_[2]["Name"] == "Cake Prince" and (L_283_[2]:FindFirstChildOfClass("Humanoid") and (L_283_[2]:FindFirstChildOfClass("Humanoid"))["Health"] > 0) then
				...
				for L_284_forvar0, L_285_forvar1 in pairs(L_1_[40]:GetChildren()) do
					...
					if L_286_[2]["Name"] == "Cookie Crafter" or L_286_[2]["Name"] == "Cake Guard" or L_286_[2]["Name"] == "Baking Staff" or L_286_[2]["Name"] == "Head Baker" then
						repeat
							...
							L_1_[14]()
						until L_1_[45]["gh"](L_286_[2]) <= 0 or not L_1_[40]:FindFirstChild(Enemy) or not(L_1_[35]["PlayerGui"]["Main"]:FindFirstChild("Quest") and L_1_[35]["PlayerGui"]["Main"].Quest.Visible) or Quest ~= nil
					end
				end
			else
...
L_1_[38] = function()
	if not noFruit then
		local bp = L_1_[35] and L_1_[35]:FindFirstChild("Backpack")
		local ch = L_1_[35] and L_1_[35].Character
		local hasChip = (ch and L_1_[45]["ffc"](ch, "Special Microchip")) or (bp and L_1_[45]["ffc"](bp, "Special Microchip"))
		
		if getgenv().UnstoringRaidFruit or not hasChip then
			return
		end
		
		if bp then
			for L_287_forvar0, L_288_forvar1 in pairs(bp:GetChildren()) do
				local L_289_ = {}
		...
		if ch then
			for L_291_forvar0, L_292_forvar1 in pairs(ch:GetChildren()) do
				local L_293_ = {}
...
task["spawn"](function()
	while L_1_[45]["wt"](1) do
		if not getgenv().UseUnifiedMeleeBuyer then xpcall(function()
			-- Legacy melee progression used several prerequisite waits with no
			-- exit condition.  A bounded pass lets the main farm continue and
			-- retry later after mastery, money, or a respawn changes.
			local legacyMeleeDeadline = tick() + 20
			if not Old_World then
				if not Black_Leg_C then
					...
					until Black_Leg_C or not getgenv().AutoFarm or tick() >= legacyMeleeDeadline
				end
				if not Electro_C then
					...
					until Electro_C or not getgenv().AutoFarm or tick() >= legacyMeleeDeadline
				end
				if not Fishman_Karate_C then
					...
					until Fishman_Karate_C or not getgenv().AutoFarm or tick() >= legacyMeleeDeadline
				end
				if not Fishman_Karate_C_M then
					...
					until Fishman_Karate_C_M or not getgenv().AutoFarm or tick() >= legacyMeleeDeadline
				end
			end
			L_1_[45]["wt"](L_1_[21])
			...
			until Super_human or not getgenv().AutoFarm or tick() >= legacyMeleeDeadline
			...
			if Black_Leg_C_M then
				...
				repeat
					...
					tries = tries + 1
				until Black_Leg_C_M or tries > 10
			end
			if not Death_Step_C_M then
				...
				repeat
					...
					tries = tries + 1
				until Death_Step_C_M or tries > 10
			end
			...
			if not Fishman_Karate_C_M then
				...
				repeat
					...
					tries = tries + 1
				until Fishman_Karate_C_M or tries > 10
			end
			if not Dragon_Claw_C_M then
				...
				repeat
					...
					tries = tries + 1
				until Dragon_Claw_C_M or tries > 10
			end
			if not Sharkman_Karate_C then
				...
				repeat
					...
					tries = tries + 1
				until Sharkman_Karate_C or tries > 10
			end
			if not Sharkman_Karate_C_M then
				...
				repeat
					...
					tries = tries + 1
				until Sharkman_Karate_C_M or tries > 10
			end
			if not Electric_Claw_C then
				...
				repeat
					...
					tries = tries + 1
				until Electric_Claw_C or tries > 10
			end
			if not Electric_Claw_C_M then
				...
				repeat
					...
					tries = tries + 1
				until Electric_Claw_C_M or tries > 10
			end
			if not Dragon_Talon_C then
				...
				repeat
					...
					tries = tries + 1
				until Dragon_Talon_C or tries > 10
			end
			if not Dragon_Talon_C_M then
				...
				repeat
					...
					tries = tries + 1
				until Dragon_Talon_C_M or tries > 10
			end
			...
			if not God_Human_C_M then
				...
				repeat
					...
					tries = tries + 1
				until God_Human_C_M or tries > 10
			end
		end, warn) end
	end
end);
	...
	while L_1_[45]["wt"]() do
		xpcall(function()
			if (getgenv())["AutoFarm"] then
				...
				if (L_1_[24]["Value"] >= 1500 or RainbowSaviour) and (not _G.LastRainbowCheck or tick() - _G.LastRainbowCheck > 60) then
					_G.LastRainbowCheck = tick()
					local netMod2 = L_1_[7]:FindFirstChild("Modules") and L_1_[7]["Modules"]:FindFirstChild("Net")
					local rf2 = netMod2 and netMod2:FindFirstChild("RF/FruitCustomizerRF")
					if rf2 then
						pcall(function()
							rf2:InvokeServer({
				...
				L_312_[1] = false
				-- Do not invoke the fruit purchase remote every frame.  It creates
				-- UI races and can starve the farming loop on slow servers.
				local canBuyFruit = not _G.LastCousinBuyAttempt or tick() - _G.LastCousinBuyAttempt >= 60
				if canBuyFruit then _G.LastCousinBuyAttempt = tick() end
				if canBuyFruit and ((L_1_[7]:WaitForChild("Remotes")):WaitForChild("CommF_")):InvokeServer("Cousin", "Buy") == 1 then
					L_1_[45]["wt"](1)
					L_1_[38]()
					AutoCloseFruitUI()
				else
					...
					if not L_312_[1] then
						...
						if L_1_[7]["Remotes"]["CommF_"]:InvokeServer("Wenlocktoad", "3") ~= -2 and (L_1_[7]["Remotes"]["CommF_"]:InvokeServer("TalkTrevor", "1") == 0 and (L_1_[45]["IsHeavenly"]() and (L_1_[22]["Value"] >= 1400 and L_1_[1]["Value"] >= 2000000))) then
							local currentRace = L_1_[35]:FindFirstChild("Data") and L_1_[35].Data:FindFirstChild("Race") and L_1_[35].Data.Race.Value or ""
							if currentRace == "Human" or currentRace == "Fishman" then
								Quest = "Evo Race V2"
								return
							end
						end
						...
						if getgenv().MeleeV2Manager then
							local target = getgenv().MeleeV2Manager:GetTarget()
							if target then
								getgenv().MeleeV2Manager:Execute(target)
								return
							end
						end
						...
						local hasUnfinishedSea2Melee = getgenv().MeleeV2Manager and ((not getgenv().MeleeV2Manager.Unlocked["Death Step"] and not getgenv().MeleeV2Manager:HasItem("Library Key")) or (not getgenv().MeleeV2Manager.Unlocked["Sharkman Karate"] and not getgenv().MeleeV2Manager:HasItem("Water Key")))
						if not hasUnfinishedSea2Melee and New_World and (CheckFindWaterKey and (L_1_[24]["Value"] >= 1500 and (L_1_[7]["Remotes"]["CommF_"]:InvokeServer("TalkTrevor", "1") == 0 and (not L_1_[45]["IsHall"]() and L_1_[45]["IsHeavenly"]())))) then
							Quest = "TravelZou"
							return
						end
						Quest = nil
...
task["spawn"](function()
	while L_1_[45]["wt"]() do
		xpcall(function()
			if Quest ~= nil then
			    if Quest == "Saber" then
			        getgenv().SetTask("MiniTask", " Status : Saber")
			        L_1_[45]["Status"](" Status : Auto Farm Level")
			    else
			...
			if Quest == "Saber" then
				...
				elseif L_430_[2]["KilledMob"] == false then
					...
					if L_434_[2] then
						for L_435_forvar0, L_436_forvar1 in pairs(L_1_[40]:GetChildren()) do
							...
							if L_437_[3]["Name"] == "Mob Leader" and L_437_[3]:FindFirstChild("HumanoidRootPart") and L_437_[3]:FindFirstChildOfClass("Humanoid") then
								repeat
									L_1_[45]["wt"]()
									...
									L_1_[14]()
								until not L_437_[3]["Parent"] or not L_437_[3]:FindFirstChildOfClass("Humanoid") or L_1_[45]["gh"](L_437_[3]) <= 0 or L_1_[45]["gi"]("Saber")
								L_1_[7]["Remotes"]["CommF_"]:InvokeServer("ProQuestProgress", "RichSon")
							end
						end
					else
				...
				elseif L_430_[2]["UsedRelic"] == false then
					...
					if L_1_[45]["ffc"](L_1_[40], "Saber Expert") then
						if getgenv().SetTask then
							getgenv().SetTask("MiniTask", "Boss | Defeating Saber Expert (Lv 200)")
						end
						for L_438_forvar0, L_439_forvar1 in pairs((game:GetService("Workspace"))["Enemies"]:GetChildren()) do
							...
							if L_440_[3]["Name"] == "Saber Expert" and L_440_[3]:FindFirstChild("HumanoidRootPart") and L_440_[3]:FindFirstChildOfClass("Humanoid") then
								repeat
									L_1_[45]["wt"]()
									if L_440_[3]:FindFirstChild("HumanoidRootPart") then
									...
									L_1_[14]()
								until not L_440_[3]["Parent"] or not L_440_[3]:FindFirstChildOfClass("Humanoid") or L_1_[45]["gh"](L_440_[3]) <= 0 or L_1_[45]["gi"]("Saber")
							end
						end
					else
    ...
    Icon = skidHubLogo,
				...
				elseif L_441_[1]["KilledIceBoss"] == false then
					if L_1_[45]["ffc"](L_1_[40], "Ice Admiral") then
						for L_442_forvar0, L_443_forvar1 in pairs(L_1_[40]:GetChildren()) do
							...
							if L_444_[2]["Name"] == "Ice Admiral" and L_444_[2]:FindFirstChild("HumanoidRootPart") and L_444_[2]:FindFirstChildOfClass("Humanoid") and L_1_[45]["gh"](L_444_[2]) > 0 then
								repeat
									L_1_[45]["wt"](.1)
									if L_444_[2]:FindFirstChild("HumanoidRootPart") then
									...
									L_1_[14]()
								until not L_444_[2]["Parent"] or not L_444_[2]:FindFirstChildOfClass("Humanoid") or L_1_[45]["gh"](L_444_[2]) <= 0 or not(getgenv())["AutoFarm"] or L_441_[1]["KilledIceBoss"]
								L_1_[45]["wt"](2)
					...
					elseif L_1_[45]["ffc"](L_1_[7], "Ice Admiral") and not L_1_[45]["ffc"](L_1_[40], "Ice Admiral") then
						-- Server hop if Ice Admiral is dead
						WindUI:Notify({
						    Title = "skid hub",
						    Content = "Ice Admiral dead. Hopping to Sea 2...",
						    Duration = 8,
						})
						L_1_[45]["wt"](.1)
						L_1_[45]["HopLowServer"](3)
					...
					if L_561_[5] ~= nil then
						...
						if L_1_[45]["ffc"](L_1_[40], L_561_[5]) then
							for L_562_forvar0, L_563_forvar1 in pairs(game["Workspace"]["Enemies"]:GetChildren()) do
								...
								if L_564_[3]["Name"] == L_561_[5] and L_1_[45]["gh"](L_564_[3]) > 0 then
									StatrMagnet = true
									repeat
										L_1_[45]["wt"]()
									...
									until not L_564_[3]["Parent"] or L_1_[45]["gh"](L_564_[3]) <= 0
									StatrMagnet = false
								end
							end
						end
					...
					if L_561_[4] ~= nil then
						...
						if game["Workspace"]["Enemies"]:FindFirstChild(L_561_[4]) then
							for L_565_forvar0, L_566_forvar1 in pairs(game["Workspace"]["Enemies"]:GetChildren()) do
								...
								if L_567_[3]["Name"] == L_561_[4] and L_1_[45]["gh"](L_567_[3]) > 0 then
									StatrMagnet = true
									repeat
										L_1_[45]["wt"]()
									...
									until not L_567_[3]["Parent"] or L_1_[45]["gh"](L_567_[3]) <= 0
									StatrMagnet = false
								end
							end
						end
				...
				repeat
					...
					if L_1_[45]["ffc"](L_1_[40], "Longma") then
						for L_568_forvar0, L_569_forvar1 in pairs(L_1_[40]:GetChildren()) do
							...
							if L_570_[2]["Name"] == "Longma" and L_570_[2]:FindFirstChild("HumanoidRootPart") and L_570_[2]:FindFirstChildOfClass("Humanoid") and L_1_[45]["gh"](L_570_[2]) > 0 then
								repeat
									L_1_[45]["wt"]()
									if L_570_[2]:FindFirstChild("HumanoidRootPart") then
									...
									L_1_[14]()
								until not L_570_[2]["Parent"] or not L_570_[2]:FindFirstChildOfClass("Humanoid") or L_1_[45]["gh"](L_570_[2]) <= 0 or not(getgenv())["AutoFarm"] or not L_1_[45]["CheckBoss"]("Longma")
							end
						end
					elseif L_1_[45]["ffc"](L_1_[7], "Longma") then
						for L_571_forvar0, L_572_forvar1 in pairs(L_1_[7]:GetChildren()) do
							...
							if L_573_[3]["Name"] == "Longma" and L_573_[3]:FindFirstChild("HumanoidRootPart") and L_573_[3]:FindFirstChildOfClass("Humanoid") and L_1_[45]["gh"](L_573_[3]) > 0 then
								repeat
									L_1_[45]["wt"]()
									if L_573_[3]:FindFirstChild("HumanoidRootPart") then
									...
									L_1_[14]()
								until not L_573_[3]["Parent"] or not L_573_[3]:FindFirstChildOfClass("Humanoid") or L_1_[45]["gh"](L_573_[3]) <= 0 or not(getgenv())["AutoFarm"] or not L_1_[45]["CheckBoss"]("Longma")
							end
						end
					else
						L_1_[45]["HopLowServer"](3)
			...
			elseif Quest == "Soul Guitar" then
				...
				elseif not Three_World then
					...
					local hc = workspace:FindFirstChild("Map") and workspace.Map:FindFirstChild("Haunted Castle"); local swamp = hc and hc:FindFirstChild("SwampWater"); if swamp and tostring(swamp.BrickColor) == "Maroon" then
					...
					elseif L_1_[7]["Remotes"]["CommF_"]:InvokeServer("GuitarPuzzleProgress", "Check") ~= nil then
						...
						if not Quest_Soul_Guitar then
							...
							wait(1)
							local gCheck1 = nil
							pcall(function() gCheck1 = L_1_[7]["Remotes"]["CommF_"]:InvokeServer("GuitarPuzzleProgress", "Check") end)
							if type(gCheck1) == "table" then
								for L_585_forvar0, L_586_forvar1 in pairs(gCheck1) do
									local L_587_ = {}
							...
							wait(2)
							local gCheck2 = nil
							pcall(function() gCheck2 = L_1_[7]["Remotes"]["CommF_"]:InvokeServer("GuitarPuzzleProgress", "Check") end)
							if type(gCheck2) == "table" then
								for L_588_forvar0, L_589_forvar1 in pairs(gCheck2) do
									local L_590_ = {}
					...
					elseif swamp and tostring(swamp.BrickColor) ~= "Maroon" then
						...
						if L_591_[1] ~= nil then
							if L_1_[45]["ffc"](L_1_[40], L_591_[1]) then
								for L_593_forvar0, L_594_forvar1 in pairs(L_1_[40]:GetChildren()) do
									...
									if L_595_[3]["Name"] == L_591_[1] and L_1_[45]["gh"](L_595_[3]) > 0 then
										repeat
											L_1_[45]["wt"]()
											...
											L_1_[14]()
										until not L_595_[3]["Parent"] or L_1_[45]["gh"](L_595_[3]) <= 0 or not(getgenv())["AutoFarm"]
									end
								end
							elseif L_1_[45]["ffc"](L_1_[7], L_591_[1]) then
								for L_596_forvar0, L_597_forvar1 in pairs(L_1_[7]:GetChildren()) do
									...
									if L_598_[1]["Name"] == L_591_[1] and L_1_[45]["gh"](L_598_[1]) > 0 then
										repeat
											L_1_[45]["wt"]()
											...
											L_1_[14]()
										until not L_598_[1]["Parent"] or L_1_[45]["gh"](L_598_[1]) <= 0 or not(getgenv())["AutoFarm"]
									end
								end
							end
						else
			...
			elseif Quest == "Cursed Dual Katana" then
				if L_1_[7]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "OpenDoor") == "opened" then
							...
							if L_1_[45]["ffc"](L_1_[35], "QuestHaze") then
								...
								elseif L_1_[45]["ffc"](L_1_[35]["QuestHaze"], Quest_Kill) and tonumber((L_1_[35]["QuestHaze"]:FindFirstChild(Quest_Kill))["Value"]) > 0 then
									for L_614_forvar0, L_615_forvar1 in pairs(L_1_[40]:GetChildren()) do
										...
										if L_616_[2]:FindFirstChildOfClass("Humanoid") and (L_1_[45]["gh"](L_616_[2]) > 0 and L_616_[2]:FindFirstChild("HazeESP")) then
											repeat
												wait(.1)
												L_1_[31](L_616_[2]["HumanoidRootPart"]["CFrame"] * CFrame["new"](0, 25, 0), 1.5)
												L_1_[14]()
											until not L_616_[2]["Parent"] or L_1_[45]["gh"](L_616_[2]) <= 0
										end
									end
						...
						elseif L_607_[2]["Good"] == -4 then
							...
							if (Vector3["new"](-5543.0805664062, 313.76550292969, -2969.4846191406) - L_1_[35]["Character"]["HumanoidRootPart"]["Position"])["Magnitude"] <= 1500 then
								for L_617_forvar0, L_618_forvar1 in pairs(L_1_[40]:GetChildren()) do
									...
									if L_619_[1]:FindFirstChildOfClass("Humanoid") and (L_1_[45]["gh"](L_619_[1]) > 0 and (L_619_[1]["HumanoidRootPart"]["Position"] - L_1_[35]["Character"]["HumanoidRootPart"]["Position"])["Magnitude"] <= 1500) then
										repeat
											wait(.3)
											L_1_[14]()
											L_1_[31](L_619_[1]["HumanoidRootPart"]["CFrame"] * CFrame["new"](0, 30, 0), 1.5)
										until not L_619_[1]["Parent"] or L_1_[45]["gh"](L_619_[1]) <= 0
									end
								end
							end
						end
							...
							if not Kill_Boss_Cake then
								if L_1_[45]["ffc"](L_1_[40], "Cake Queen") then
									for L_620_forvar0, L_621_forvar1 in pairs(L_1_[40]:GetChildren()) do
										...
										if L_622_[2]["Name"] == "Cake Queen" and (L_1_[45]["gh"](L_622_[2]) > 0 and not Kill_Boss_Cake) then
											repeat
												wait(.3)
											...
											until not L_622_[2]["Parent"] or L_1_[45]["gh"](L_622_[2]) <= 0
											Kill_Boss_Cake = true
											wait(1)
										end
									end
									...
									elseif Ceyma_HeavenlyDimension then
										...
										if L_1_[40]:FindFirstChildOfClass("Model") then
											for L_623_forvar0, L_624_forvar1 in pairs(L_1_[40]:GetChildren()) do
												...
												if L_625_[2]:FindFirstChild("HumanoidRootPart") and (L_625_[2]:FindFirstChildOfClass("Humanoid") and ((L_1_[19]["Map"]:FindFirstChild("HeavenlyDimension"))["WorldPivot"]["Position"] - L_625_[2]["HumanoidRootPart"]["Position"])["Magnitude"] <= 1000) then
													if L_1_[45]["gh"](L_625_[2]) > 0 then
														repeat
															wait()
															L_1_[31](L_625_[2]["HumanoidRootPart"]["CFrame"] * CFrame["new"](0, 30, 0), 1.5)
															L_1_[14]()
														until not L_625_[2]["Parent"] or L_1_[45]["gh"](L_625_[2]) <= 0
													end
												end
											end
								...
								elseif ((L_1_[19]["Map"]:FindFirstChild("HellDimension"))["WorldPivot"]["Position"] - L_1_[35]["Character"]["HumanoidRootPart"]["Position"])["Magnitude"] <= 1200 then
									...
									if L_1_[40]:FindFirstChildOfClass("Model") then
										for L_626_forvar0, L_627_forvar1 in pairs(L_1_[40]:GetChildren()) do
											...
											if L_628_[1]:FindFirstChild("HumanoidRootPart") and (L_628_[1]:FindFirstChildOfClass("Humanoid") and ((L_1_[19]["Map"]:FindFirstChild("HellDimension"))["WorldPivot"]["Position"] - L_628_[1]["HumanoidRootPart"]["Position"])["Magnitude"] <= 1000) then
												if L_1_[45]["gh"](L_628_[1]) > 0 then
													repeat
														wait()
														L_1_[31](L_628_[1]["HumanoidRootPart"]["CFrame"] * CFrame["new"](0, 30, 0), 1.5)
														L_1_[14]()
													until not L_628_[1]["Parent"] or L_1_[45]["gh"](L_628_[1]) <= 0
												end
											end
										end
									elseif not L_1_[40]:FindFirstChildOfClass("Model") then
					...
					elseif L_607_[2]["Good"] == 3 then
						...
						wait(1)
						L_1_[45]["click"](L_1_[35]["PlayerGui"]["Main"]["Dialogue"])
					local cDoor = L_1_[19]:FindFirstChild("Map") and L_1_[19].Map:FindFirstChild("Turtle") and L_1_[19].Map.Turtle:FindFirstChild("Cursed") and L_1_[19].Map.Turtle.Cursed:FindFirstChild("BossDoor"); elseif L_607_[2]["Good"] == 4 and (L_607_[2]["Evil"] == 4 and cDoor["Position"]["Y"] > 584) then
						L_1_[14]()
						repeat
							wait(.1)
						...
						wait(1)
						L_1_[45]["click"](L_1_[35]["PlayerGui"]["Main"]["Dialogue"])
					elseif cDoor and cDoor.Position.Y <= 584 then
						local L_629_ = {}
						if L_1_[4]:FindFirstChild("     ") then
							L_1_[4]["     "]["Enabled"] = true
						end
						...
						if (Vector3["new"](-12297.5605, 598.726013, -6532.96436) - L_1_[35]["Character"]["HumanoidRootPart"]["Position"])["Magnitude"] <= 100 then
							...
							for L_636_forvar0, L_637_forvar1 in pairs(L_1_[40]:GetChildren()) do
								...
								if L_638_[3]["Name"] == "Cursed Skeleton Boss" then
									...
									until not L_638_[3]["Parent"] or L_1_[45]["gh"](L_638_[3]) <= 0
	...
	while L_1_[45]["wt"]() do
		xpcall(function()
			for L_642_forvar0, L_643_forvar1 in pairs(L_1_[40]:GetChildren()) do
				...
				if L_1_[45]["ffc"](L_644_[1], "Humanoid") and (L_1_[45]["gh"](L_644_[1]) <= 0 and L_1_[45]["ffc"](L_644_[1], "HumanoidRootPart")) then
					if L_1_[45]["gh"](L_644_[1]) <= 0 then
						L_644_[1]:Destroy()
					end
				end
			end
			if L_1_[45]["ffc"](L_1_[35]["Character"], "Black Leg") then
				local blTool = L_1_[35]["Character"]:FindFirstChild("Black Leg")
				local blLvl = blTool and blTool:FindFirstChild("Level") and blTool.Level.Value or 0
				if blLvl >= 150 then
					if not _G.LastDiableJambe or (tick() - _G.LastDiableJambe >= 25) then
						_G.LastDiableJambe = tick()
						L_1_[2]:SendKeyEvent(true, "V", false, game)
						task.wait(0.1)
						L_1_[2]:SendKeyEvent(false, "V", false, game)
					end
			...
			if L_1_[45]["ffc"](L_1_[35]["Character"], "HumanoidRootPart") and not L_1_[45]["ffc"](L_1_[35]["Character"]["HumanoidRootPart"], "Lock") then
				local L_645_ = {}
				L_645_[1] = L_1_[35]["Character"]:FindFirstChildOfClass("Humanoid")
	...
	while task["wait"]() do
		if Quest ~= "Cursed Dual Katana" and (Quest ~= "Evo Race V2" and (Quest ~= "Evo Race V1" and not SROP)) then
			...
			if L_650_[1] <= 1 then
				...
				if L_1_[28] >= 120 and (Quest ~= "Cursed Dual Katana" and (Quest ~= "Evo Race V2" and (Quest ~= "Evo Race V1" and not SROP))) then
